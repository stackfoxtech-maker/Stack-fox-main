...
for (const [label, item, type] of [
  ...
  checks.push([
    `  priced from catalogue (${line?.price} == ${item.price})`,
...
checks.push([
  `tampered price rejected (line is ${line?.price}, not 1)`,
  line?.price === service.price,
]);
checks.push([
  `tampered name rejected (line is "${line?.name}")`,
  line?.name === service.name,
]);

const unknown = await post("/cart/add", {
  itemId: "definitely-not-real",
  itemType: "service",
});
...
checks.push([
  `cart persists (${fetched.data?.cart?.items?.length} lines)`,
  fetched.data?.cart?.items?.length === 4,
]);
...
checks.push([
  `clear -> ${cleared.s}, ${cleared.b?.data?.cart?.items?.length} lines`,
  cleared.b?.data?.cart?.items?.length === 0,
]);
