...
for (const [who, tok, shouldSee] of [
  ["A", A.token, true],
  ["B", B.token, false],
] as const) {
...
checks.push([
  `B probing A's project id -> ${probe.status}`,
  probe.status === 404,
  "expect 404",
]);
...
checks.push([
  `B probing A's milestones -> ${probeMs.status}`,
  probeMs.status === 404,
  "expect 404",
]);
