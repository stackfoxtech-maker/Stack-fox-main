...
if (!regBody?.data?.accessToken) {
  console.error("register failed:", regBody);
  process.exit(1);
}
const { accessToken: token, orgId } = {
  accessToken: regBody.data.accessToken,
  orgId: regBody.data.user.orgId,
};
...
const proj = await prisma.project.create({
  data: {
    id: `SF-KIT-${String(stamp).slice(-8)}`,
    name: "Handover Kit Demo",
    engagementId: eng.id,
    serviceId: svc!.id,
    configSnapshot: {},
    status: "COMPLETED",
...
await prisma.file.create({
  data: {
    ...
    storageKey: `files/${proj.id}/source.zip`,
    sizeBytes: 10240,
    mimeType: "application/zip",
    uploadedBy: regBody.data.user.id,
  },
});
const vault = await prisma.credentialVault.create({
  data: {
    projectId: proj.id,
    systemName: "Production hosting",
...
checks.push([
  `spend.totals.paid = ${d?.spend?.totals?.paid} (expect 59000)`,
  d?.spend?.totals?.paid === 59000,
]);
checks.push([`spend.series has 6 months`, d?.spend?.series?.length === 6]);
checks.push([
  `timeline.totals.approved = ${d?.timeline?.totals?.approved} (expect 2)`,
  d?.timeline?.totals?.approved === 2,
]);
checks.push([
  `timeline onTimePct = ${d?.timeline?.totals?.onTimePct} (expect 100)`,
  d?.timeline?.totals?.onTimePct === 100,
]);
checks.push([
  `revisions roundsUsed = ${d?.revisions?.totals?.roundsUsed} (expect 2)`,
  d?.revisions?.totals?.roundsUsed === 2,
]);
...
checks.push([
  `lists the seeded project`,
  (hp.b.data ?? []).some((p: any) => p.id === proj.id),
]);
...
checks.push([
  `kit has 1 credential (metadata only)`,
  kit.b?.data?.credentials?.length === 1,
]);
checks.push([
  `credential blob NOT exposed`,
  kit.b?.data?.credentials?.[0]?.encryptedBlob === undefined,
]);
checks.push([
  `checklist computed`,
  Array.isArray(kit.b?.data?.checklist) && kit.b.data.checklist.length === 5,
]);
checks.push([
  `readyToAccept = ${kit.b?.data?.readyToAccept} (expect true)`,
  kit.b?.data?.readyToAccept === true,
]);
...
checks.push([
  ...
  Array.isArray(audited?.accessLog) && (audited.accessLog as any[]).length === 1,
]);
