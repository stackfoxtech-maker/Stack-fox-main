...
async function register(tag: string) {
  ...
  if (!r.b?.data?.accessToken)
    throw new Error(`register ${tag} failed: ${JSON.stringify(r.b)}`);
  return {
    email,
...
async function makeStaff(role: string) {
  ...
  if (!r.b?.data?.accessToken)
    throw new Error(`staff login failed: ${JSON.stringify(r.b)}`);
  return {
    email,
    token: r.b.data.accessToken as string,
    userId: r.b.data.user.id as string,
...
check(
  `A lists its own project`,
  (aProjects.b?.data ?? []).some((p: any) => p.id === project.id),
  "expect 1",
);
...
check(
  `client POST /engagements -> ${clientCreatesEngagement.s}`,
  clientCreatesEngagement.s === 403,
  "expect 403",
);
...
const selfApprove = await call(
  "POST",
  `/workspaces/${workspace.id}/se-approve`,
  clientA.token,
);
...
const seEscalate = await call("PATCH", `/admin/users/${se.userId}`, se.token, {
  role: "ADMIN",
});
check(
  `SE promoting self to ADMIN -> ${seEscalate.s}`,
  seEscalate.s === 403,
  "expect 403",
);

const adminSelfDemote = await call("PATCH", `/admin/users/${admin.userId}`, admin.token, {
  role: "PM",
});
check(
  `admin changing own role -> ${adminSelfDemote.s}`,
  adminSelfDemote.s === 409,
  "expect 409",
);

const bogusRole = await call("PATCH", `/admin/users/${clientA.userId}`, admin.token, {
  role: "GOD_MODE",
});
...
check(
  `admin update honours allowlist -> ${massAssign.s}`,
  massAssign.s === 200,
  "expect 200",
);
check(
  `email not overwritable`,
  afterMass?.email === clientB.email,
...
check(
  `admin sees tenant A's project`,
  (adminProjects.b?.data ?? []).some((p: any) => p.id === project.id),
  "internal is cross-tenant",
);
const adminFinance = await call("GET", "/finance/ar-aging", admin.token);
check(
  `admin GET /finance/ar-aging -> ${adminFinance.s}`,
  adminFinance.s === 200,
  "expect 200",
);
...
check(
  `token rejected after logout -> ${afterLogout.s}`,
  afterLogout.s === 401,
  "expect 401",
);
...
await prisma.referral.create({
  data: {
    referrerType: "CLIENT",
    referrerId: clientA.userId,
    code: `AC${stamp}`.slice(0, 10),
    referredEmail: `friend-${stamp}@example.com`,
    status: "PENDING",
