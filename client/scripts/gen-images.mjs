...
if (!KEY) {
  console.error("Missing GEMINI_API_KEY (put it in client/.env)");
  process.exit(1);
}
...
const IMAGES = [
  {
    ...
    ar: "4:3",
    prompt:
      `An Indian woman founder in her early 30s working calmly at a light oak desk in a bright minimal studio, ` +
  ...
  },
  {
    ...
    ar: "3:2",
    prompt:
      `Three people collaborating around a light oak table with laptops closed and paper sketches spread out, ` +
  ...
  },
  {
    ...
    ar: "3:2",
    prompt:
      `Wide interior of a small calm software studio in India: a few empty oak desks, warm task lamps, ` +
  ...
  },
  {
    ...
    ar: "3:2",
    prompt:
      `Close-up of hands sketching website wireframe boxes with a pencil on warm off-white paper beside a ` +
  ...
  },
  {
    ...
    ar: "16:9",
    prompt:
      `Overhead flat-lay of pale wooden modular building blocks and square tiles arranged into a neat grid ` +
  ...
  },
  {
    ...
    ar: "4:3",
    prompt:
      `A tidy small-business packing bench: open cardboard boxes of fresh organic vegetables and pantry jars ` +
  ...
  },
  {
    ...
    ar: "4:3",
    prompt:
      `A calm modern clinic reception area in warm tones: light wood counter, soft seating, a plant, ` +
      `morning light, nobody in frame or one blurred figure. Reassuring and clean. ${LOOK}`,
  },
  {
    name: "portfolio-realestate",
    ar: "4:3",
    prompt:
      `A small architectural model house and a set of keys on a warm oak table beside a rolled floor plan ` +
  ...
  },
  {
    ...
    ar: "4:3",
    prompt:
      `A young student in a warm sunlit library nook reading from a tablet held like a book, stacked books, ` +
  ...
  },
  {
    ...
    ar: "4:3",
    prompt:
      `A restaurant kitchen pass in warm light: two freshly plated modern Indian dishes on the counter, ` +
  ...
  },
  {
    ...
    ar: "4:3",
    prompt:
      `An intimate event hall being set up: rows of pale wooden chairs, warm string lights overhead, ` +
  ...
  },
  {
    ...
    ar: "16:9",
    prompt:
      `A warm Indian high-street shopfront at golden hour — a small independent boutique with a clean awning, ` +
  ...
  },
  {
    ...
    ar: "3:2",
    prompt:
      `A person at a warm home-office desk mid friendly video call, laptop open away from camera, gesturing ` +
...
for (const img of pick) {
  ...
  try {
    ...
    await sharp(raw)
      .resize(MAX_W, null, { withoutEnlargement: true })
      .webp({ quality: WEBP_Q })
      .toFile(webp);
