...
export default function Reviews() {
  ...
  useEffect(() => {
    api
      .get('/reviews/my')
      .then((r) => setCompletedReviews(r.data.data || []))
    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <Star
          key={s}
          size={14}
          className={s <= Math.round(rating) ? 'text-amber-400 fill-amber-400' : 'text-warm-200'}
        />
      ))}
    </div>
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      api
        ...
        .catch(() => toast.error('Failed to refresh pending reviews.'));
    } catch {
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 bg-warm-50 rounded-xl p-1">
        {[
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-medium transition ${tab === t.key ? 'bg-white text-fox-500 shadow-sm' : 'text-warm-500 hover:text-warm-700'}`}
          >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-warm-500">
                      By {r.reviewer?.name || r.reviewerId?.slice(-4)} &middot;{' '}
        ...
        <div className="space-y-3">
          {pendingRequests.length === 0 ? (
            ...
            pendingRequests.map((r) => (
              <div
                key={r.projectId || r.revieweeId}
                className="bg-white rounded-2xl border border-warm-200 p-5 flex items-center justify-between"
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-fox-500/10 flex items-center justify-center text-fox-500 text-xs font-bold">
                    {(r.revieweeName || '?')[0]}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-warm-900">
                      {r.revieweeName || 'Unknown'}
                    </p>
                ...
                <div className="text-right">
                  <button
                    onClick={() => {
                      setReviewFor(r.revieweeId);
                      setTab('submit');
                    }}
                    className="block text-xs text-fox-500 hover:underline"
                  >
                    Write Review
                  </button>
                </div>
      ...
      {tab === 'submit' && (
        <form
          ...
          className="bg-white rounded-2xl border border-warm-200 p-6 space-y-5"
        >
          <div>
            ...
            >
              {pendingRequests.map((p) => (
                <option key={p.revieweeId || p.projectId} value={p.revieweeId}>
                  {p.revieweeName} - {p.projectId || p.period}
                </option>
              ))}
            </select>
          </div>
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((s) => (
                ...
                >
                  <Star
                    size={28}
                    className={`${(formHover || formRating) >= s ? 'text-amber-400 fill-amber-400' : 'text-warm-200'} transition-colors`}
                  />
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Share your observations about their performance..."
          ...
          <button
            type="submit"
            disabled={!formRating || !formText || submitting}
            className="px-6 py-2.5 rounded-xl bg-fox-500 text-white text-sm font-medium hover:bg-fox-600 transition disabled:opacity-40 flex items-center gap-2"
          >
