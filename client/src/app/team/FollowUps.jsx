...
const baseSuggestedMessages = {
  ...
  WhatsApp:
    "Hi {name}, hope you're doing well. Following up on our conversation about {business}. Here is the information I promised. Let me know if you have any questions!",
  Email:
    'Dear {name}, I hope this email finds you well. I wanted to follow up on our discussion about {business}. Please find the attached proposal for your review.',
  Meeting:
    'Hi {name}, I would like to schedule a meeting to discuss {business}. Please let me know your availability this week.',
...
const categoryFollowUpTips = {
  ...
  restaurant:
    'Share a sample restaurant website with online table booking. Mention how other restaurants reduced missed bookings by 50%.',
  ...
  hotel:
    'Share a sample hotel website with booking system. Mention how direct bookings save 20-25% commission.',
  clinic:
    'Share a sample clinic website with appointment booking. Mention how clinics reduced no-shows by 30% with automated reminders.',
  hospital:
    'Share a sample hospital website with department pages and online appointments. Emphasize trust and credibility.',
  'real-estate':
    'Share a sample real estate website with property listings. Mention how agencies increased buyer enquiries by 25%.',
  school:
    'Share a sample school website with admission forms. Mention how schools increased admissions with online presence.',
  'car-dealer':
    'Share a sample car dealer website with inventory and EMI calculator. Mention how dealers increased enquiries by 20%.',
};
...
const toRow = (f) => {
  ...
  return {
    ...
    action:
      f.note ||
      `${CHANNEL_TO_TYPE[f.channel] || 'Follow up'} with ${f.lead?.company || f.lead?.name || 'the lead'}`,
    notes: f.note || '',
...
export default function FollowUps() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([apiGet('/followups', { status: 'PENDING' }), apiGet('/leads', { limit: 200 })])
      .then(([fu, ld]) => {
  ...
  const todayFollowUps = useMemo(
    () => rows.filter((f) => f.date <= today && !f.completed),
    [rows, today],
  );
  ...
  const selectedPitch = selectedFollowUp
    ? getPitch(selectedFollowUp.category, { name: selectedFollowUp.business })
    : null;
  ...
  const shareWhatsApp = (fu) => {
    ...
    const text = encodeURIComponent(
      (pitch?.whatsappPitch || 'Hi ' + fu.leadName + '!') + '\n\n' + fu.action,
    );
        ...
        <>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-warm-900">
                    Due &amp; overdue ({todayFollowUps.length})
                  </h3>
                </div>
              </div>
              <div className="space-y-3">
                {todayFollowUps.length === 0 && (
                  <p className="text-sm text-warm-500 text-center py-6">Nothing due right now</p>
                )}
                {todayFollowUps.map((fu) => (
                  <div
                    key={fu.id}
                    className="p-4 rounded-xl border border-warm-100 hover:bg-warm-50 transition"
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() => complete(fu.id)}
                          className="p-1 rounded-lg hover:bg-warm-100 text-warm-400 hover:text-success-500 transition"
                          title="Mark done"
                        >
                          <CheckCircle size={16} />
                        </button>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-warm-400">
                        {formatDate(fu.date)} {fu.time}
                      </span>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => shareWhatsApp(fu)}
                          className="text-xs text-success-600 hover:text-success-700 flex items-center gap-1"
                        >
                        ...
                        <button
                          onClick={() =>
                            navigator.clipboard?.writeText(
                              getSuggestedMessage(fu.type, fu.leadName, fu.business),
                            )
                          }
            ...
            <div className="bg-white rounded-2xl border border-warm-200 p-6">
              ...
              <div className="space-y-3">
                {upcomingFollowUps.length === 0 && (
                  <p className="text-sm text-warm-500 text-center py-6">No upcoming follow-ups</p>
                )}
                {upcomingFollowUps.map((fu) => (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedFollowUp?.id === fu.id
                        ? 'border-fox-300 bg-fox-50'
          ...
          {selectedFollowUp && selectedPitch && (
            <div className="bg-fox-50 border border-fox-200 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-semibold text-fox-800">
                    Smart Follow-Up for {selectedFollowUp.business}
                  </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {form.type && form.leadId && (
            <div className="p-4 rounded-xl bg-fox-50 border border-fox-100">
              ...
              <p className="text-sm text-fox-600 whitespace-pre-line">
                {getSuggestedMessage(form.type, formLead?.label, formLead?.label)}
              </p>
            </div>
          )}
