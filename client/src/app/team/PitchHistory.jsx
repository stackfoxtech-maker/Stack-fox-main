...
const categoryColors = {
  ...
  'real-estate': 'badge-warning',
  school: 'badge-fox',
  'car-dealer': 'badge-danger',
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-warm-500 text-sm mt-1">
          Track which pitches work best for each category
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-5">
          ...
          <p className="text-2xl font-bold text-warm-900">
            {history.length > 0
              ? Math.round(
                  (history.filter((h) => h.outcome === 'won' || h.outcome === 'shared').length /
                    history.length) *
                    100,
                )
              : 0}
        ...
        <div className="space-y-3">
          {topCategories.length === 0 && (
            <p className="text-sm text-warm-500 text-center py-4">
              No pitch history yet. Start using pitches in Pitch Studio to see your performance
              here.
            </p>
          )}
            ...
            <div key={cat} className="flex items-center gap-4 p-3 rounded-xl bg-warm-50">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge variant={categoryColors[cat]?.replace('badge-', '') || 'neutral'}>
                    {data.wins} wins
                  </Badge>
                </div>
                <div className="w-full bg-warm-200 rounded-full h-2">
                  <div
                    className="bg-fox-500 h-2 rounded-full transition-all"
                    style={{ width: `${Math.round((data.wins / data.total) * 100)}%` }}
                  />
                </div>
                <p className="text-xs text-warm-500 mt-1">
                  {data.total} total uses | {Math.round((data.wins / data.total) * 100)}% success
                  rate
                </p>
              </div>
            </div>
      ...
      <div className="bg-white rounded-2xl border border-warm-200 p-6">
        ...
        <div className="space-y-3">
          {filtered.length === 0 && (
            <p className="text-sm text-warm-500 text-center py-6">No pitch history yet</p>
          )}
          {filtered
            .slice(-10)
            .reverse()
            .map((h, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-center justify-between p-4 rounded-xl border border-warm-100 hover:bg-warm-50 transition"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-warm-900">
                        {pitch?.categoryName || h.category}
                      </p>
                      <p className="text-xs text-warm-500">
                        {new Date(h.date).toLocaleDateString()} | Outcome: {h.outcome}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => sharePitch(h)}
                    ...
                    <Badge
                      variant={
                        h.outcome === 'won' || h.outcome === 'shared'
                          ? 'success'
                          : h.outcome === 'lost'
                            ? 'danger'
                            : 'neutral'
