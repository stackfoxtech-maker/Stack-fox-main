...
export default function Knowledge() {
  ...
  useEffect(() => {
    api
      .get('/blog')
      .then((r) => setArticles((r.data.data || []).map(mapPostToArticle)))
  ...
  const handleSuggest = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/blog/suggest', {
        title: suggestTitle,
        content: suggestBody,
        category: category === 'all' ? 'Development' : category,
      });
    ...
    <div className="space-y-5">
      ...
      {showSuggest && (
        <form
          ...
          className="bg-white rounded-2xl border border-warm-200 p-6 space-y-3"
        >
          <input
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={submitting}
              className="px-4 py-2 rounded-xl bg-fox-500 text-white text-sm font-medium hover:bg-fox-600 transition disabled:opacity-50"
            >
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-warm-100 text-warm-600 text-sm font-medium hover:bg-warm-200 transition"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="flex gap-2">
        {categories.map((c) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition ${category === c.key ? 'bg-fox-500 text-white' : 'bg-warm-50 text-warm-600 hover:bg-warm-100'}`}
          >
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-fox-500 text-white text-sm font-medium hover:bg-fox-600 transition"
      >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          {filtered.map((a) => (
            <div
              key={a.id}
              className="bg-white rounded-2xl border border-warm-200 p-5 hover:shadow-sm transition cursor-pointer group"
            >
              <div className="flex items-start justify-between mb-2">
                <Badge variant={categoryColor[a.category] || 'default'}>{a.category}</Badge>
                <ExternalLink
                  size={14}
                  className="text-warm-300 group-hover:text-fox-500 transition"
                />
              </div>
