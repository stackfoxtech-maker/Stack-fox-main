...
export default function SalesCall() {
  ...
  const toggleItem = (idx) => {
    setCheckedItems((prev) => ({
      ...prev,
      [`${currentStep}-${idx}`]: !prev[`${currentStep}-${idx}`],
    }));
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-warm-500 text-sm mt-1">
          Step-by-step guided sales call assistant with category-specific scripts
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-warm-200 p-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          ...
          <div className="flex items-end">
            <Button variant="outline" onClick={resetCall} className="w-full">
              <RotateCcw size={16} /> Reset Call
            </Button>
          </div>
        </div>
      </div>
      ...
      {!category ? (
        <div className="bg-warning-50 border border-warning-200 rounded-2xl p-6 text-center">
          <p className="text-warning-700 font-medium">
            Select a category to load a category-specific sales script
          </p>
          <p className="text-warning-600 text-sm mt-1">
            The script will adapt to the selected business type with relevant questions, objections,
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-warm-200 p-6">
            ...
            <div className="w-full bg-warm-100 rounded-full h-2 mb-6">
              <div
                className="bg-fox-500 h-2 rounded-full transition-all duration-300"
                ...
                <div className="space-y-2">
                  ...
                  {step.questions.map((q, idx) => (
                    ...
                    >
                      <div
                        className={cn(
                          'w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition',
                          checkedItems[`${currentStep}-${idx}`]
                  ...
                  {step.points.map((point, idx) => (
                    <div
                      ...
                      className={cn(
                        ...
                        checkedItems[`${currentStep}-${idx}`]
                          ? 'bg-success-50 border-success-200'
                          : 'bg-fox-50 border-fox-100 hover:border-fox-200',
                      )}
                    >
                      <div
                        className={cn(
                          'w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold',
                          checkedItems[`${currentStep}-${idx}`]
              ...
              {step.objections && step.objections.length > 0 && (
                <div className="space-y-3">
                  ...
                  {step.objections.map((obj, idx) => (
                    <div
                      key={idx}
                      className="p-4 rounded-xl bg-warning-50 border border-warning-100"
                    >
                  ...
                  {step.closingQuestions.map((q, idx) => (
                    <div
                      ...
                      className={cn(
                        ...
                        checkedItems[`${currentStep}-${idx}`]
                          ? 'bg-success-50 border-success-200'
                          : 'bg-success-50 border-success-100 hover:border-success-200',
                      )}
                    >
                      <CheckCircle
                        size={16}
          ...
          <div className="bg-white rounded-2xl border border-warm-200 p-6">
            ...
            <textarea
              ...
              className="w-full border border-warm-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fox-500/30 resize-none"
              rows={4}
            />
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-success-800">
                    Call Complete! Recommended Next Actions
                  </h3>
                  <p className="text-sm text-success-600">
                    Based on your call, here are the suggested next steps:
                  </p>
                </div>
              </div>
