...
export function SEQueue() {
  ...
  useEffect(() => {
    api.get('/admin/se-queue').then((r) => {
      const data = r.data.data || r.data.items || r.data;
      setItems(Array.isArray(data) ? data : []);
    }).catch((e) => {
      if (e.response?.status === 403) {
        setError('SE Queue is admin-only. You do not have access to view this data.');
      } else {
        setError('Failed to load SE queue.');
        toast.error('Failed to load SE queue.');
      }
    }).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="p-6">
      ...
      {loading ? (
        ...
        <p className="text-sm text-orange-600 text-center py-8">{error}</p>
      ) : items.length === 0 ? (
        <p className="text-sm text-warm-400 text-center py-8">No items in SE queue.</p>
      ) : (
        <div className="space-y-3">
          {items.map((t) => (
            <div key={t.id || t._id} className="border rounded-xl p-4 flex justify-between items-center">
              <div>
                <div className="font-medium">{t.title || t.name || 'Untitled'}</div>
                <div className="text-xs text-gray-500">Confidence: {t.confidence ? Math.round(t.confidence * 100) + '%' : 'N/A'}</div>
              </div>
              <div className="text-right">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${assigned ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
