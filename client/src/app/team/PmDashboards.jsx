...
export function Queue() {
  ...
  useEffect(() => {
    api
      .get('/projects', { params: { status: 'PENDING' } })
      .then((r) => {
        const items = r.data.data || r.data.items || r.data;
        setProjects(Array.isArray(items) ? items : []);
      })
      .catch(() => toast.error('Failed to load project queue.'))
  ...
  return (
    <div className="p-6">
      <p className="text-sm font-semibold text-orange-600 mb-2">
        H1 &middot; Order and Project Queue
      </p>
      <h1 className="text-2xl font-bold mb-6">Incoming Projects</h1>
      <p className="text-gray-600 mb-6">
        New orders awaiting kickoff. Sorted by earliest commitment date.
      </p>
      {queue.length === 0 ? (
        <EmptyState
          title="No projects in queue"
          description="Projects with DRAFT or PENDING status will appear here."
        />
      ) : (
            ...
            >
              <div>
                <div className="font-semibold">
                  {p.projectNumber || p.id || p._id}: {p.title || p.service?.name || 'Untitled'}
                </div>
                <div className="text-sm text-gray-500">
                  Status: {p.status} &middot; {p.milestones?.length || 0} milestones
                </div>
              </div>
              <div className="text-right">
                <Link
                  to={'/builder?service=' + (p.id || p._id)}
...
export function Sprints() {
  ...
  return (
    <div className="p-6">
      <p className="text-sm font-semibold text-orange-600 mb-2">
        H2 &middot; Sprint and Task Management
      </p>
          ...
          {sprintData.map((s) => (
            <div key={s.name} className="border rounded-xl p-4">
              <div className="flex justify-between font-semibold">
                <span>{s.name}</span>
                <span className={s.status === 'completed' ? 'text-green-700' : 'text-orange-700'}>
                  {s.status}
                </span>
              </div>
              <div className="mt-2 bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{ width: s.tasks > 0 ? Math.round((s.done / s.tasks) * 100) + '%' : '0%' }}
                />
              </div>
...
export function Resources() {
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-gray-600 mb-6">
        Open workload per person against a comfortable ceiling of {meta.capacity} tasks. Red means
        over-allocated.
      </p>
        ...
        <>
          ...
          {rows.length === 0 ? (
            <EmptyState
              ...
              description="Team members appear here once they have accounts."
            />
          ) : (
            <div className="space-y-3">
              {rows.map((r) => (
                  ...
                  <div className="flex justify-between items-start gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div className="text-xs text-gray-500">
                        {r.openTasks} open
                        {r.overdue > 0 && (
                          <span className="text-red-600"> &middot; {r.overdue} overdue</span>
                        )}
                      </div>
                    </div>
                  </div>
    ...
    <div className="p-6">
      ...
      {openTickets.length === 0 ? (
        <EmptyState
          ...
          description="All tickets are resolved or none have been created yet."
        />
      ) : (
        <div className="space-y-3">
          {openTickets.map((b) => (
            <div key={b.id || b._id} className="border rounded-xl p-4 flex justify-between">
              <div>
                <span
                  className={`text-xs font-medium px-2 py-0.5 rounded-full ${b.severity === 'Critical' || b.severity === 'P1' ? 'bg-red-100 text-red-700' : b.severity === 'High' || b.severity === 'P2' ? 'bg-orange-100 text-orange-700' : 'bg-yellow-100 text-yellow-700'}`}
                >
                  {b.severity || 'Medium'}
                </span>
              ...
              <span className="text-sm text-gray-500">
                {b.project?.projectNumber || b.projectId || '—'}
              </span>
            </div>
          ))}
        </div>
            ...
            <div key={i.id || i._id} className="border rounded-xl p-4 flex justify-between">
              <div>
                ...
                <span className="text-sm text-gray-500 ml-2">
                  {i.project?.projectNumber || i.engagement?.projectNumber || '—'}
                </span>
              </div>
              <div className="text-right">
...
export function Clients() {
  ...
  projects.forEach((p) => {
    const orgName =
      p.engagement?.client?.name || p.engagement?.org?.name || p.org?.name || 'Unknown Client';
    const clientId = p.engagement?.clientId || p.engagement?.orgId || p.orgId || 'unknown';
    if (!clientMap[clientId])
      clientMap[clientId] = { name: orgName, id: clientId, projects: 0, status: p.status };
  ...
  return (
    <div className="p-6">
      ...
      {clients.length === 0 ? (
        <EmptyState
          title="No clients"
          description="Clients will appear here once projects are created."
        />
      ) : (
        <div className="space-y-3">
          {clients.map((c) => (
            <div key={c.id} className="border rounded-xl p-4 flex justify-between">
              ...
              <div className="text-right">
                ...
                <span
                  className={`text-xs font-medium ${c.status === 'active' || c.status === 'IN_PROGRESS' ? 'text-green-700' : 'text-yellow-700'}`}
                >
                  {c.status}
                </span>
              </div>
...
export function Analysis() {
  ...
  useEffect(() => {
    Promise.all([api.get('/analytics/overview'), api.get('/analytics/revenue')])
  ...
  const metrics = analytics
    ? [
        { metric: 'Active projects', value: analytics.totalProjects || 0 },
        {
          ...
          value:
            'Rs ' +
            (analytics.totalRevenue ? (analytics.totalRevenue / 100000).toFixed(1) + 'L' : '0'),
        },
  ...
  return (
    <div className="p-6">
      <p className="text-sm font-semibold text-orange-600 mb-2">
        H7 &middot; Analytics and Reporting
      </p>
                ...
                {revenue.map((r, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div className="text-[10px] text-gray-500">
                      Rs {(r.value / 1000).toFixed(0)}K
                    </div>
                    <div
                      className="w-full bg-orange-500 rounded-t-md transition-all"
                      style={{
                        height:
                          maxRevenue > 0 ? Math.max(4, (r.value / maxRevenue) * 100) + '%' : '4px',
                      }}
                    />
