...
export default function Tasks() {
  ...
  Object.keys(columnConfig).forEach((k) => {
    columns[k] = tasks.filter((t) => t.status === k);
  });
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-1 bg-warm-100 rounded-lg p-0.5">
          <button
            ...
            className={cn(
              ...
              view === 'board' ? 'bg-white shadow-sm text-warm-900' : 'text-warm-500',
            )}
          >
            Board
          </button>
          <button
            ...
            className={cn(
              'px-3 py-1 rounded-md text-xs font-medium transition-colors',
              view === 'list' ? 'bg-white shadow-sm text-warm-900' : 'text-warm-500',
            )}
          >
            List
          </button>
      ...
      {tasks.length === 0 ? (
        <EmptyState
          ...
          description="Tasks will appear here when assigned to you by a project manager."
        />
      ) : view === 'board' ? (
            ...
            <div key={status} className="min-w-[260px] w-[260px] shrink-0">
              ...
              <div className="space-y-2">
                {(columns[status] || []).map((task) => (
                  <div
                    key={task._id}
                    className="bg-white rounded-xl border border-warm-200 p-3 shadow-sm"
                  >
                    ...
                    {task.dueDate && (
                      <p
                        className={cn(
                          'text-[10px]',
                          new Date(task.dueDate) < new Date() ? 'text-danger-500' : 'text-warm-400',
                        )}
                      >
                      ...
                      {Object.keys(columnConfig)
                        ...
                        .map((s) => (
                          <button
                            ...
                            className="text-[10px] text-fox-500 hover:underline"
                          >
        ...
        <div className="space-y-2">
          {tasks.map((t) => (
            <div
              key={t._id}
              className="bg-white rounded-xl border border-warm-200 p-4 flex items-center gap-3"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-warm-500">
                  {t.project?.projectNumber} {t.dueDate && `· ${formatDate(t.dueDate)}`}
                </p>
              </div>
              <Badge
                variant={cn(
