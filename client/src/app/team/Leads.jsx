...
const followUpSuggestions = {
  'New Lead': {
    type: 'Call',
    action: 'Introduce yourself and understand their current website situation',
    timing: 'Within 24 hours',
  },
  Contacted: {
    type: 'WhatsApp',
    action: 'Send a sample website link and pricing details',
    timing: 'Within 48 hours',
  },
  Interested: {
    type: 'Call',
    action: 'Schedule a demo or site visit to show the website proposal',
    timing: 'Within 24 hours',
  },
  'Meeting Scheduled': {
    type: 'Meeting',
    action: 'Prepare pitch presentation and proposal for the meeting',
    timing: 'Before meeting',
  },
  'Demo Completed': {
    type: 'Email',
    action: 'Send proposal and follow up with any additional questions',
    timing: 'Within 24 hours',
  },
  'Proposal Sent': {
    type: 'Call',
    action: 'Follow up on proposal review and address any concerns',
    timing: 'Within 3 days',
  },
  Negotiation: { type: 'Call', action: 'Discuss final terms and close the deal', timing: 'ASAP' },
  Won: {
    type: 'Email',
    action: 'Send welcome email and project kick-off details',
    timing: 'Immediately',
  },
  'Not Interested': {
    type: 'WhatsApp',
    action: 'Send a thank you message and keep the door open for future',
    timing: 'Within 1 week',
  },
  Lost: {
    type: 'Email',
    action: 'Send a feedback request to understand why they chose another provider',
    timing: 'Within 1 week',
  },
  'Follow Up Later': {
    type: 'WhatsApp',
    action: 'Set a reminder to follow up at the agreed time',
    timing: 'As scheduled',
  },
};
...
export default function Leads() {
  ...
  const selectedPitch = selectedLead
    ? getPitch(selectedLead.category, {
        name: selectedLead.businessName,
        city: selectedLead.location,
      })
    : null;
  ...
  const getPriorityColor = (p) =>
    ({ High: 'danger', Medium: 'warning', Low: 'neutral' })[p] || 'neutral';
  const getStatusColor = (s) =>
    ({
      ...
      'Demo Completed': 'warning',
      'Proposal Sent': 'fox',
      Negotiation: 'warning',
      Won: 'success',
      'Not Interested': 'neutral',
      Lost: 'danger',
      'Follow Up Later': 'neutral',
    })[s] || 'neutral';
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-4 py-2.5 rounded-xl border border-warm-200 text-sm focus:outline-none focus:ring-2 focus:ring-fox-500/30"
          >
          ...
          {selectedLead && selectedPitch ? (
            <>
              <div className="bg-white rounded-2xl border border-warm-200 p-5">
                ...
                <p className="text-sm text-warm-700 leading-relaxed mb-3">
                  {selectedPitch.mainPitch}
                </p>
                <div className="flex items-center gap-2">
                  <Button
                    size="sm"
                    onClick={() => {
                      navigator.clipboard?.writeText(selectedPitch.mainPitch);
                      toast.success('Pitch copied');
                    }}
                  >
                    <Copy size={14} /> Copy
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => sharePitch(selectedLead)}>
              ...
              {suggestion && (
                <div className="bg-success-50 border border-success-100 rounded-2xl p-5">
                  ...
                  <p className="text-xs text-success-600 mb-1">
                    Suggested: <span className="font-medium">{suggestion.type}</span> —{' '}
                    {suggestion.timing}
                  </p>
              ...
              <div className="bg-white rounded-2xl border border-warm-200 p-5">
                ...
                <div className="space-y-2">
                  <Button
                    ...
                    onClick={() => {
                      navigator.clipboard?.writeText(selectedPitch.shortPitch);
                  ...
                  <Button
                    variant="outline"
                    className="w-full justify-between"
                    onClick={() => sharePitch(selectedLead)}
                  >
            ...
            <div className="bg-warm-50 border border-warm-200 rounded-2xl p-6 text-center">
              ...
              <p className="text-sm text-warm-500">
                Select a lead to view its auto-generated pitch and follow-up suggestion.
              </p>
            </div>
          )}
        </div>
