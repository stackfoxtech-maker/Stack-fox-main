...
export default function Pipeline() {
  ...
  const getPriorityColor = (p) =>
    ({ High: 'border-l-danger-500', Medium: 'border-l-warning-500', Low: 'border-l-warm-300' })[
      p
    ] || 'border-l-warm-300';
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4">
        {pipelineStages.map((stage) => {
          ...
          return (
            <div
              key={stage.id}
              className={cn('flex-shrink-0 w-72 rounded-2xl border p-4', stage.color)}
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-sm text-warm-800">{stage.label}</h3>
                <span className="text-xs font-medium text-warm-500 bg-white/60 px-2 py-0.5 rounded-lg">
                  {stageLeads.length}
                </span>
              </div>
              <div className="space-y-3">
                {stageLeads.map((lead) => (
                  <div
                    key={lead.id}
                    className={cn(
                      'bg-white rounded-xl border-l-4 p-3 shadow-sm hover:shadow-md transition',
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-mono font-medium text-warm-700">
                        ...
                        {pipelineStages
                          ...
                          .map((s) => (
                            <button
                              ...
                              className="text-xs px-2 py-1 rounded-md bg-warm-50 text-warm-600 hover:bg-warm-100 transition disabled:opacity-50"
                            >
                              {s.label}
                            </button>
                          ))}
