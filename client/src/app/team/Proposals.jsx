...
function priceRange(...strings) {
  const nums = strings.filter(Boolean).join(' ').replace(/,/g, '').match(/\d+/g)?.map(Number) ?? [];
...
const websitePackages = [
  {
    ...
    name: 'Basic Website',
    price: '₹15,000 - ₹25,000',
    features: ['5-7 Pages', 'Mobile Responsive', 'Contact Form', 'Google Maps', 'Basic SEO'],
  },
  {
    id: 'professional',
...
const seoPackages = [
  {
    ...
    name: 'Basic SEO',
    price: '₹5,000/month',
    features: ['On-Page SEO', 'Google Business Profile', 'Basic Keywords', 'Monthly Report'],
  },
  {
    id: 'advanced-seo',
    name: 'Advanced SEO',
...
const categoryRecommendations = {
  gym: {
    ...
    seo: 'advanced-seo',
    marketing: 'social',
    reason: 'Gyms need online booking and class schedules to capture trial enquiries.',
  },
  restaurant: {
    website: 'professional',
    seo: 'advanced-seo',
    marketing: 'social',
    reason: 'Restaurants need menu display, table booking, and local SEO for "near me" searches.',
  },
  cafe: {
    website: 'professional',
    seo: 'advanced-seo',
    marketing: 'social',
    reason: 'Cafes need ambiance photos, menu display, and Instagram integration for foot traffic.',
  },
  hotel: {
    website: 'enterprise',
    seo: 'advanced-seo',
    marketing: 'google-ads',
    reason: 'Hotels need booking system, room showcase, and Google Ads for direct bookings.',
  },
  clinic: {
    website: 'professional',
    seo: 'advanced-seo',
    marketing: 'google-ads',
    reason: 'Clinics need appointment booking, service pages, and local SEO for patient enquiries.',
  },
  hospital: {
    website: 'enterprise',
    seo: 'advanced-seo',
    marketing: 'google-ads',
    reason:
      'Hospitals need comprehensive information, department pages, and trust-building features.',
  },
  'real-estate': {
    website: 'enterprise',
    seo: 'advanced-seo',
    marketing: 'google-ads',
    reason:
      'Real estate needs property listings, virtual tours, and advanced search for serious buyers.',
  },
  school: {
    website: 'professional',
    seo: 'advanced-seo',
    marketing: 'social',
    reason: 'Schools need admission forms, faculty profiles, and result showcase for parents.',
  },
  'car-dealer': {
    website: 'enterprise',
    seo: 'advanced-seo',
    marketing: 'google-ads',
    reason: 'Car dealers need inventory catalog, EMI calculator, and lead capture for buyers.',
  },
};
  ...
  const shareWhatsApp = () => {
    ...
    const text = encodeURIComponent(
      'Proposal for ' +
        ...
        pitch.shortPitch +
        '\n\nWebsite: ' +
        (selectedWebsite?.name || 'TBD') +
        '\nSEO: ' +
        (selectedSEO?.name || 'TBD') +
        '\n\n' +
        pitch.mainPitch,
    );
            ...
            <div key={p.id} className="flex items-center justify-between px-5 py-3">
              <div>
                ...
                <p className="text-xs text-warm-500">
                  {p.lead?.company || p.lead?.name} · {formatDate(p.createdAt)}
                </p>
              </div>
              <Badge
                variant={
          ...
          {recommendation && (
            <div className="bg-info-50 border border-info-100 rounded-xl p-4 mb-4">
              <p className="text-sm font-medium text-info-700">
                Recommended for {businessCategories.find((c) => c.id === form.category)?.name}:
              </p>
              <p className="text-sm text-info-600 mt-1">{recommendation.reason}</p>
              <p className="text-xs text-info-500 mt-1">
                Website: {recommendation.website} | SEO: {recommendation.seo} | Marketing:{' '}
            ...
            <div>
              <label className="block text-sm font-medium text-warm-700 mb-1.5">
                ...
                <span className="text-warm-400 font-normal">
                  (links the proposal to a pipeline lead)
                </span>
              </label>
              <select
                value={form.leadId}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, websitePackage: e.target.value })}
                  className="input-fx"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, seoPackage: e.target.value })}
                  className="input-fx"
                >
                  <option value="">Select package</option>
                  {seoPackages.map((p) => (
              ...
              <div>
                ...
                <select
                  value={form.marketingPackage}
                  onChange={(e) => setForm({ ...form, marketingPackage: e.target.value })}
                  className="input-fx"
                >
                  <option value="">Select package</option>
              ...
              <div className="sm:col-span-2">
                ...
                <Textarea
                  ...
                  placeholder="Describe the client's current challenges..."
                />
              </div>
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-warm-700 mb-1.5">
        ...
        <div className="bg-white rounded-2xl border border-warm-200 overflow-hidden">
          <div className="bg-fox-500 text-white p-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-fox-100 text-sm">
                {form.businessName} — {new Date().toLocaleDateString()}
              </p>
            </div>
            <div className="flex items-center gap-2">
              {savedId && (
                <button
                  ...
                  disabled={busy}
                  className="px-3 py-2 rounded-lg bg-white text-fox-600 text-sm font-semibold hover:bg-fox-50 transition flex items-center gap-1.5 disabled:opacity-60"
                >
          ...
          <div className="p-8 space-y-8 max-w-4xl">
            ...
            {pitch && (
              <div className="bg-fox-50 border border-fox-100 rounded-xl p-5">
                <h4 className="text-xs font-semibold text-fox-700 uppercase tracking-wide mb-2">
                  Personalized Pitch
                </h4>
            ...
            <div>
              ...
              <p className="text-sm text-warm-700 leading-relaxed">
                {form.challenges ||
                  'Client requires a professional digital presence to attract more customers and manage enquiries efficiently.'}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-warm-700 leading-relaxed">
                {form.solution ||
                  'A professional website with enquiry management, SEO optimization, and marketing support.'}
              </p>
            </div>
              ...
              {form.marketingPackage && (
                <div className="p-4 rounded-xl bg-success-50 border border-success-100">
                  ...
                  <p className="text-sm text-success-600">
                    {marketingPackages.find((p) => p.id === form.marketingPackage)?.name}
                  </p>
                  <p className="text-sm font-medium text-success-700">
                    {marketingPackages.find((p) => p.id === form.marketingPackage)?.price}
                  </p>
                </div>
              )}
            </div>
            ...
            {pitch && (
              <div className="bg-success-50 border border-success-100 rounded-xl p-5">
                <h4 className="text-xs font-semibold text-success-700 uppercase tracking-wide mb-2">
                  Why This Works for {pitch.categoryName}
                </h4>
                <p className="text-sm text-warm-700 leading-relaxed mb-3">
                  {pitch.competitorAdvantage}
                </p>
