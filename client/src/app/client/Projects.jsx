...
const msStatusColor = {
  PENDING: 'text-warm-400',
  IN_PROGRESS: 'text-warning-500',
  IN_REVIEW: 'text-fox-500',
  APPROVED: 'text-success-500',
  REVISION: 'text-danger-500',
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link to="/app/client/projects" className="p-2 hover:bg-warm-100 rounded-lg">
          <ArrowLeft size={18} />
        </Link>
                ...
                <div key={ms.id || i} className="flex items-start gap-3 p-3 rounded-xl bg-warm-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {ms.deliverables?.length > 0 && (
                      <p className="text-xs text-warm-500 mt-0.5">{ms.deliverables.join(', ')}</p>
                    )}
      ...
      {project.techStack?.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {project.techStack.map((t) => (
            <span key={t} className="badge-fx badge-neutral text-xs">
              {t}
            </span>
          ))}
        </div>
    ...
    <div className="space-y-4">
      ...
      {projects.length === 0 ? (
        <EmptyState
          ...
          description="Your projects will appear here once a quote is converted."
          action={
            <Link to="/builder" className="btn-fox text-sm px-4 py-2">
              Browse Services
            </Link>
          }
        />
        ...
        <div className="space-y-3">
          {projects.map((p) => (
            <Link
              key={p._id}
              to={`/app/client/projects/${p._id}`}
              className="block bg-white rounded-2xl border border-warm-200 p-5 hover:shadow-card transition-shadow"
            >
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <h3 className="font-medium text-warm-900">{p.name}</h3>
                  <p className="text-xs text-warm-500 mt-0.5">
                    {p.id} &middot; {formatDate(p.createdAt)}
                  </p>
                </div>
              ...
              <div className="flex gap-4 mt-3 text-xs text-warm-500">
                <span>
                  Service: <span className="font-mono text-warm-700">{p.service?.name || '–'}</span>
                </span>
                <span>
                  Status: <span className="font-mono text-success-700">{capitalize(p.status)}</span>
                </span>
              </div>
            </Link>
          ))}
