    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2 text-sm text-warm-500">
          <Link to="/app/client/engagements" className="hover:text-fox-500">
            Engagements
          </Link>
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              <span className="text-warm-500">Methodology</span>
              <p className="font-medium">{selected.methodology}</p>
            </div>
            <div>
              <span className="text-warm-500">Start</span>
              <p className="font-medium">{formatDate(selected.createdAt)}</p>
            </div>
            <div>
              <span className="text-warm-500">End</span>
              <p className="font-medium">{selected.endsAt ? formatDate(selected.endsAt) : '–'}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      {engagements.length === 0 ? (
        <EmptyState
          ...
          description="Your active engagements will appear here once a project is contracted."
        />
      ) : (
        <div className="space-y-3">
          {engagements.map((eng) => (
            <Link
              key={eng.id}
              to={`/app/client/engagements/${eng.id}`}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-medium text-warm-900">
                    {eng.model} – {eng.methodology}
                  </p>
                  <p className="text-sm text-warm-500">{eng.id}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Badge variant={eng.status === 'ACTIVE' ? 'success' : 'neutral'}>
                  {eng.status}
                </Badge>
