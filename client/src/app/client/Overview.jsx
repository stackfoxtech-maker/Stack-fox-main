...
const projectProgress = (p) => {
  ...
  if (totalPct > 0) {
    return Math.round(
      (ms.filter(done).reduce((s, m) => s + (m.paymentPct || 0), 0) / totalPct) * 100,
    );
  }
...
const StatCard = ({ label, value, icon: Icon, color, to }) => (
  <Link
    to={to}
    className="bg-white rounded-md border border-warm-200 p-5 hover:shadow-card transition-shadow group"
  >
      ...
      <div className="bg-white rounded-md border border-warm-200 p-5">
        ...
        {projects.length === 0 ? (
          <EmptyState
            ...
            description="Create a quote from the Service Builder to start your first project."
            action={
