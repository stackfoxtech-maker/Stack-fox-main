  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const taskData =
          taskRaw?.tasks || taskRaw?.items || (Array.isArray(taskRaw) ? taskRaw : []);
        setTasks(
          taskData.map((t) => ({
            id: t._id,
            name: t.title || t.name,
        ...
        const uniqueAssignees = [
          ...new Map(
            taskData
              .filter((t) => t.assignee)
              .map((t) => [t.assignee._id || t.assignee.name, t.assignee]),
          ).values(),
        ];
        setTeam(
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-warm-200 p-6">
        ...
        <div className="w-full bg-warm-100 rounded-full h-2.5">
          <div
            className="bg-fox-500 h-2.5 rounded-full transition-all"
      ...
      <div className="flex gap-1 bg-warm-50 rounded-xl p-1">
        {tabs.map((t) => (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition ${tab === t.key ? 'bg-white text-fox-500 shadow-sm' : 'text-warm-500 hover:text-warm-700'}`}
          >
        ...
        {tab === 'team' &&
          (team.length === 0 ? (
            <EmptyState
              ...
              description="Team members will appear once tasks are assigned."
            />
          ) : (
            <div className="space-y-3">
              {team.map((m) => (
                <div key={m.name} className="flex items-center gap-3 py-2">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${getAvatarColor(m.name)}`}
                  >
                    {getInitials(m.name)}
                  </div>
                  <div>
