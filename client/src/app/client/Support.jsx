    ...
    <div className="space-y-4">
      ...
      {tickets.length === 0 ? (
        <EmptyState
          icon={LifeBuoy}
          title="No tickets"
          description="Submit a ticket if you need help."
        />
      ) : (
        <div className="space-y-3">
          {tickets.map((t) => (
            <button
              key={t._id}
              onClick={() =>
                api.get(`/support/${t._id}`).then((r) => setSelected(r.data.data.ticket))
              }
              className="w-full text-left bg-white rounded-xl border border-warm-200 p-4 hover:shadow-card transition-shadow"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-medium text-warm-900">{t.subject}</p>
                  <p className="text-xs text-warm-500 mt-0.5">
                    {t.ticketNumber} &middot; {formatDate(t.createdAt)}
                  </p>
                </div>
                <Badge variant={getStatusBadge(t.status)?.replace('badge-', '')}>
                  {capitalize(t.status)}
                </Badge>
              </div>
            </button>
      ...
      <Modal
        ...
        title={selected?.ticketNumber || 'Ticket'}
        size="lg"
      >
        {selected && (
          <div className="space-y-4">
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto">
              {selected.replies?.map((r, i) => (
                <div
                  key={i}
                  className={`p-3 rounded-xl text-sm ${r.sender?._id === selected.client?._id || r.sender?.role === 'client' ? 'bg-fox-50 ml-8' : 'bg-warm-50 mr-8'}`}
                >
                  <p className="font-medium text-xs text-warm-700 mb-1">
                    {r.sender?.name || 'Support'}
                  </p>
            ...
            {!['resolved', 'closed'].includes(selected.status) && (
              <div className="flex gap-2">
                ...
                />
                <Button variant="primary" size="icon" onClick={sendReply}>
                  <Send size={18} />
                </Button>
              </div>
            )}
