...
function resumeState(q) {
  ...
  if (validUntil && validUntil.getTime() < Date.now()) {
    return {
      canResume: false,
      reason: 'This quote has expired. Rebuild it to get current pricing.',
    };
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-warm-500">
            Track your service requests and approved estimates.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {quotes.map((q) => {
            ...
            return (
              <div
                key={q._id}
                className="bg-white rounded-[2rem] border border-warm-200 overflow-hidden hover:shadow-lg transition-shadow"
              >
                <div className="p-6 md:p-8">
                  <div className="flex flex-wrap items-start justify-between gap-6 mb-8">
                    <div className="flex gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge variant={getStatusBadge(q.status)?.replace('badge-', '')}>
                            {capitalize(q.status)}
                          </Badge>
                        </div>
                    ...
                    <div className="text-right">
                      <div className="text-[10px] font-bold text-warm-400 uppercase tracking-widest mb-1">
                        Grand Total
                      </div>
                      <div className="font-mono text-2xl font-black text-fox-500">
                        {formatINR(q.total)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-warm-50 rounded-3xl p-6 mb-6">
                    <h4 className="text-xs font-bold text-warm-400 uppercase tracking-widest mb-4">
                      Included Services
                    </h4>
                      ...
                      {q.items?.map((item, i) => (
                        <div key={i} className="flex justify-between items-center text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            {item.quantity > 1 && (
                              <span className="text-xs text-warm-400">×{item.quantity}</span>
                            )}
                          </div>
