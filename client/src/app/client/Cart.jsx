...
export default function Cart() {
  usePageTitle('Cart');
  const { items, subtotal, gstAmount, total, itemCount, removeItem, updateQuantity, clearCart } =
    useCartStore();
    ...
    return (
      <div className="space-y-4">
        ...
        <EmptyState
          ...
          description="Add services from the Service Builder to get started."
          action={
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-3">
          {items.map((item) => (
            <div
              key={item._id}
              className="bg-white rounded-xl border border-warm-200 p-4 flex items-center gap-4"
            >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-7 h-7 rounded-lg border border-warm-200 flex items-center justify-center hover:bg-warm-50 transition-colors"
                >
                  <Minus size={12} />
                </button>
              ...
              <div className="text-right min-w-[100px]">
                ...
                {item.quantity > 1 && (
                  <p className="text-[10px] text-warm-400">{formatINR(item.price)} each</p>
                )}
              </div>
          ...
          <div className="bg-white rounded-2xl border border-warm-200 p-5 sticky top-24">
            ...
            <div className="space-y-2 text-sm mb-4">
              <div className="flex justify-between">
                ...
                <span className="font-mono text-warm-900">{formatINR(subtotal)}</span>
              </div>
              <div className="flex justify-between">
            ...
            <p className="text-[10px] text-warm-400 mb-4">
              All prices are indicative. Final quote may vary based on project complexity.
            </p>
            ...
            <Link
              to="/builder"
              className="block text-center text-xs text-fox-500 hover:underline mt-3"
            >
              Continue browsing
            </Link>
          </div>
