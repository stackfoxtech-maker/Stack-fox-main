      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-10 text-center">
          ...
          <button
            ...
            className="mt-5 px-5 py-2 rounded-xl bg-warm-100 text-warm-700 text-sm font-medium hover:bg-warm-200 transition"
          >
            Submit Another
          </button>
        </div>
                    ...
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          ...
                          size={14}
                          className={
                            s <= f.rating ? 'text-amber-400 fill-amber-400' : 'text-warm-200'
                          }
                        />
                      ))}
                    </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-white rounded-2xl border border-warm-200 p-6">
          ...
          {projects.length === 0 ? (
            <EmptyState
              ...
              description="You can leave feedback once you have an active project."
            />
          ) : (
            <div className="space-y-2">
              {projects.map((p) => (
                <label
                  key={p._id}
                  className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border transition ${selected === p._id ? 'border-fox-500 bg-fox-500/5' : 'border-warm-100 hover:bg-warm-50'}`}
                >
                  <input
                    type="radio"
                    name="project"
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((s) => (
              ...
              >
                <Star
                  size={32}
                  className={`${(hover || rating) >= s ? 'text-amber-400 fill-amber-400' : 'text-warm-200'} transition-colors`}
                />
              </button>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-6">
          <label className="block text-sm font-medium text-warm-700 mb-2">
            How likely are you to recommend StackFox? (0-10)
          </label>
          <input
            type="range"
            min={0}
            ...
            className="w-full accent-fox-500"
          />
        ...
        <button
          type="submit"
          disabled={!rating || submitting || projects.length === 0}
          className="w-full py-3 rounded-xl bg-fox-500 text-white font-medium text-sm hover:bg-fox-600 transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
