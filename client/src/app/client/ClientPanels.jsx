    ...
    <div className="p-6">
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description={
            unreadOnly ? 'You are all caught up.' : 'Events appear here as your project progresses.'
          }
        />
      ) : (
          ...
          {items.map((e) => (
            ...
            >
              ...
              <div className="text-[11px] text-warm-400 mt-1.5 font-mono">
                {e.actor === 'system' || e.actor === 'SYSTEM' ? 'StackFox' : e.actor} &middot;{' '}
                {e.code}
              </div>
            </div>
          ))}
    ...
    <div className="p-6">
              ...
              <div key={c.id} className="border border-warm-200 rounded-xl p-4 bg-white">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-warm-400 mt-1.5">
                      Raised {formatDate(c.createdAt)}
                    </div>
                  </div>
                  <span
                    className={`text-xs font-medium px-2 py-1 rounded-full shrink-0 ${CR_STATUS_STYLE[status] ?? 'bg-warm-100 text-warm-600'}`}
                  >
                    {status.replace(/_/g, ' ')}
                  </span>
                </div>
                  ...
                  <div className="mt-3 rounded-xl bg-warm-50 p-3">
                    <div className="flex flex-wrap gap-4 text-sm">
                      ...
                      {c.timelineDelta != null && (
                        <span>
                          Timeline impact:{' '}
                          <span className="font-mono font-semibold">+{c.timelineDelta} days</span>
                        </span>
                      )}
                    </div>
      ...
      >
        <div className="space-y-4">
          ...
          />
          <Button variant="primary" onClick={create} isLoading={submitting}>
            Submit request
          </Button>
        </div>
      </Modal>
                ...
                <div key={p.projectId}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-mono text-warm-600 shrink-0 ml-2">
                      ...
                      {p.lateCount > 0 && (
                        <span className="text-danger-600 ml-2">{p.lateCount} late</span>
                      )}
                    </span>
                  </div>
          ...
          {hasRevisions ? (
            <>
              <div className="grid grid-cols-3 gap-3 mb-4">
                <Stat
                  label="Rounds used"
                  value={`${revisions.totals.roundsUsed}/${revisions.totals.roundsIncluded}`}
                />
        ...
        >
          <div className="grid grid-cols-2 gap-3 mb-4">
            <Stat
              ...
              value={minutesToText(engagement?.totals?.avgResponseMinutes)}
            />
            <Stat
              label="Avg resolution"
              value={minutesToText(engagement?.totals?.avgResolutionMinutes)}
            />
            <Stat
              label="SLA met"
              value={
                engagement?.totals?.slaMetPct != null ? `${engagement.totals.slaMetPct}%` : '—'
              }
              tone={
                engagement?.totals?.slaMetPct != null && engagement.totals.slaMetPct < 80
                  ? 'danger'
                  : 'success'
              }
            />
            <Stat
              label="Your rating"
              value={
                engagement?.totals?.avgRating != null ? `${engagement.totals.avgRating}/5` : '—'
              }
              hint={
                engagement?.totals?.responses
                  ? `${engagement.totals.responses} response(s)`
                  : 'No feedback yet'
              }
            />
          </div>
...
export function Handover() {
  ...
  const dispute = async () => {
    if (!disputeReason.trim()) {
      toast.error('Tell us what is outstanding.');
      return;
    }
    setBusy('dispute');
    try {
        ...
        <>
            ...
            <div className="space-y-2">
              {kit.checklist.map((c) => (
                <div key={c.key} className="flex items-center gap-3 text-sm">
                  ...
                  {c.detail && (
                    <span className="text-xs text-warm-400 ml-auto font-mono">{c.detail}</span>
                  )}
                </div>
              ))}
            </div>
            ...
            {kit.deliverables.length === 0 ? (
              ...
              <div className="space-y-2">
                {kit.deliverables.map((d) => (
                  <div
                    key={d.id}
                    className="flex items-center justify-between gap-3 border border-warm-100 rounded-xl p-3"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-warm-500">
                          {d.sizeBytes ? `${(d.sizeBytes / 1024).toFixed(0)} KB · ` : ''}v
                          {d.version} · {formatDate(d.createdAt)}
                        </div>
                      </div>
                    </div>
                    <Button
          ...
          <div className="bg-white border border-warm-200 rounded-2xl p-5 mb-6">
            ...
            {kit.credentials.length === 0 ? (
              <p className="text-sm text-warm-400">
                No credentials have been handed over for this project.
              </p>
            ) : (
              <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {c.accessedAt && (
                            <div className="text-xs text-warm-500">
                              Last viewed {formatDate(c.accessedAt)}
                            </div>
                          )}
                        </div>
                      </div>
          ...
          <div className="bg-warm-50 rounded-2xl p-6">
            ...
            {warranty?.active ? (
              <p className="text-sm text-warm-600">
                Active until <strong>{formatDate(warranty.expiresAt)}</strong> —{' '}
                {warranty.daysRemaining} day
                {warranty.daysRemaining === 1 ? '' : 's'} remaining. Any bug you report in this
                window is fixed free of charge. Raise it under{' '}
                <Link to="/app/client/support" className="text-fox-600 hover:underline">
                  Support
                </Link>
                .
              </p>
              ...
              <p className="text-sm text-warm-600">
                Starts when you accept the handover above, and runs for {kit.handover.warrantyDays}{' '}
                days.
              </p>
            )}
          </div>
