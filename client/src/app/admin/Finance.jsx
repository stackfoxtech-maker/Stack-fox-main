...
export default function Finance() {
  ...
  const handleMarkPaid = async () => {
    ...
    setSaving(true);
    try {
      await api.patch(`/invoices/${paidModal.invoice.id}/utr`, {
        utr: utr.trim(),
        ...
        {error ? (
          ...
          <div className="p-8">
            <EmptyState
              icon={IndianRupee}
              title="No invoices"
              description="Invoices will appear here as projects progress."
            />
          </div>
        ) : (
        ...
        <div className="space-y-4">
          <div className="bg-warm-50 rounded-xl p-3">
            ...
            <p className="font-mono font-bold text-sm">
              {paidModal.invoice?.invoiceNumber || paidModal.invoice?.id}
            </p>
            <p className="text-xs text-warm-500 mt-1">
              Client: {paidModal.invoice?.org?.name || '–'}
            </p>
          </div>
          <Input
            label="UTR Number"
          ...
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="ghost" onClick={closePaidModal}>
              Cancel
            </Button>
            <Button variant="primary" onClick={handleMarkPaid} isLoading={saving}>
              Confirm Paid
            </Button>
          </div>
        </div>
