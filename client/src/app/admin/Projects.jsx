      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-warm-500">
              {pid} &middot; Service: {project.service?.name || project.serviceId}
            </p>
          </div>
          <Badge variant={getStatusBadge(project.status)?.replace('badge-', '')}>
            {capitalize((project.status || '').replace(/_/g, ' '))}
          </Badge>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {PROJECT_STATUSES.map((s) => (
              <Button
                key={s}
                size="sm"
                variant={project.status === s ? 'primary' : 'ghost'}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-warm-50 rounded-xl p-4">
            <div className="text-xs text-warm-500">Milestones</div>
            <div className="font-mono font-semibold mt-1">{total}</div>
          </div>
          <div className="bg-warm-50 rounded-xl p-4">
            <div className="text-xs text-warm-500">Approved</div>
            <div className="font-mono font-semibold mt-1 text-success-700">{completed}</div>
          </div>
          <div className="bg-warm-50 rounded-xl p-4">
            <div className="text-xs text-warm-500">Progress</div>
            <div className="font-mono font-semibold mt-1">
              {total ? Math.round((completed / total) * 100) : 0}%
            </div>
          </div>
          <div className="bg-warm-50 rounded-xl p-4">
            <div className="text-xs text-warm-500">Created</div>
            <div className="font-mono font-semibold mt-1">{formatDate(project.createdAt)}</div>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-warm-200 overflow-hidden">
          <h3 className="font-semibold text-warm-900 px-5 py-4 border-b border-warm-100">
            Milestones
          </h3>
          {!project.milestones?.length ? (
            <p className="p-5 text-sm text-warm-400">No milestones yet.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-warm-500 bg-warm-50">
                  ...
                  {(project.milestones || []).map((ms, i) => (
                    <tr key={ms.id || i} className="border-t border-warm-100 hover:bg-warm-50/50">
                      ...
                      <td className="px-5 py-3">
                        <Badge
                          variant={getStatusBadge(ms.status)?.replace('badge-', '') || 'neutral'}
                        >
                          {capitalize((ms.status || '').replace(/_/g, ' '))}
                        </Badge>
                      </td>
                      <td className="px-5 py-3">
                        <Button
        ...
        <div className="space-y-3">
          {visible.map((p) => {
            ...
            return (
              <Link
                key={pid}
                to={`/app/admin/projects/${pid}`}
                className="block bg-white rounded-xl border border-warm-200 p-4 hover:shadow-card transition-shadow"
              >
                <div className="flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-warm-900">{p.name || pid}</p>
                    <p className="text-xs text-warm-500">
                      {pid} &middot; {p.service?.name || p.serviceId} &middot;{' '}
                  ...
                  <Badge variant={getStatusBadge(p.status)?.replace('badge-', '')}>
                    {capitalize((p.status || '').replace(/_/g, ' '))}
                  </Badge>
                </div>
              </Link>
            );
