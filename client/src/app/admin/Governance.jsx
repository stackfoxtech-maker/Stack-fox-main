...
export function Flags() {
  ...
  const create = async () => {
    if (!form.id) {
      toast.error('Flag ID is required.');
      return;
    }
    setSaving(true);
    try {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-warm-500">
            Boolean and variant flags that gate platform behavior.
          </p>
        </div>
            ...
            <div key={f.id} className="p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-warm-500">
                  {f.description || 'No description'} &middot; {f.type}
                </div>
              </div>
              <button
                onClick={() => toggle(f)}
                className={cn(
                  ...
                  f.defaultValue ? 'bg-fox-500' : 'bg-warm-200',
                )}
              >
                <span
                  className={cn(
                    'absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform',
                    f.defaultValue ? 'translate-x-5' : 'translate-x-0.5',
                  )}
                />
              </button>
            </div>
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title="New feature flag">
        <div className="space-y-4">
          <Input
            ...
            placeholder="whatsapp_commerce_enabled"
          />
          <Input
            label="Description"
          ...
          />
          <Button variant="primary" onClick={create} isLoading={saving}>
            Create
          </Button>
        </div>
      </Modal>
...
export function Pricing() {
  ...
  const create = async () => {
    if (!form.key || !form.rate) {
      toast.error('Key and rate are required.');
      return;
    }
    setSaving(true);
    try {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-warm-500">
            Versioned per-point and per-role rates powering estimates and T&amp;M billing.
          </p>
        </div>
        <Button variant="primary" size="sm" onClick={() => setOpen(true)}>
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title="New rate card entry">
        <div className="space-y-4">
          <Select
            ...
            onChange={(e) => setForm({ ...form, type: e.target.value })}
            options={[
              { value: 'ROLE', label: 'Role (₹/hour)' },
              { value: 'POINT', label: 'Point (₹/point)' },
            ]}
          />
          <Input
            label="Key"
...
export function Templates() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ key: '', subject: '', body: '', ctaLabel: '', ctaUrlTpl: '' });
    setOpen(true);
  };
  const openEdit = (t) => {
    setEditing(t.key);
  ...
  const save = async () => {
    if (!form.key || !form.body) {
      toast.error('Key and body are required.');
      return;
    }
    setSaving(true);
    try {
      if (editing)
        await api.patch(`/admin/notification-templates/${encodeURIComponent(editing)}`, form);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-warm-500">
            One entry per event code &middot; channel, e.g. TICKET_RAISED.email.
          </p>
        </div>
      ...
      >
        <div className="space-y-4">
          {!editing && (
            <Input
              label="Key (event_code.channel)"
              value={form.key}
              onChange={(e) => setForm({ ...form, key: e.target.value })}
              placeholder="TICKET_RAISED.email"
            />
          )}
          <Input
          ...
          />
          <Input
            label="CTA URL template"
            value={form.ctaUrlTpl || ''}
            onChange={(e) => setForm({ ...form, ctaUrlTpl: e.target.value })}
          />
...
export function Compliance() {
  ...
  const markFiled = async (item) => {
    try {
      await api.patch(`/admin/compliance/${item.id}`, {
        status: 'FILED',
  ...
  const create = async () => {
    if (!form.orgId || !form.dueDate) {
      toast.error('Org ID and due date are required.');
      return;
    }
    setSaving(true);
    try {
      await api.post('/admin/compliance', {
        ...form,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-warm-500">
            Statutory filing deadlines per organisation — GST, TDS, GSTR-1, SOFTEX.
          </p>
        </div>
        <Button variant="primary" size="sm" onClick={() => setOpen(true)}>
            ...
            <div key={c.id} className="p-4 flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-warm-900">
                  {c.type} &middot; {c.org?.name || c.orgId}
                </div>
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-warm-500">
          Orders and accounts held for manual review before proceeding.
        </p>
      </div>
      {loading ? (
        <div className="flex justify-center py-16">
          ...
          {items.map((s) => (
            <div key={s.id} className="p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-warm-500">
                  {s.orgId ? `Org ${s.orgId}` : s.userId ? `User ${s.userId}` : '—'} &middot;{' '}
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => resolve(s, 'PASS')}
                  className="text-xs text-success-600 font-medium hover:underline"
                >
                  Clear
                </button>
                <button
                  onClick={() => resolve(s, 'FAIL')}
