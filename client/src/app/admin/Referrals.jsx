...
const payoutVariant = (status) =>
  status === 'PAID' || status === 'CONVERTED'
    ? 'success'
    : status === 'EXPIRED' || status === 'DUPLICATE'
      ? 'neutral'
      : 'warning';
  ...
  const statCards = [
    {
      ...
      value: stats?.total ?? referrals.length,
      icon: Users,
    ...
    {
      label: 'Top Referrer',
      value: topReferrer?.name ? topReferrer.name.split(' ')[0] + '.' : '—',
      icon: Award,
      ...
      <div className="flex gap-1 bg-warm-50 rounded-xl p-1">
        {['leaderboard', 'payouts'].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition capitalize ${tab === t ? 'bg-white text-fox-500 shadow-sm' : 'text-warm-500 hover:text-warm-700'}`}
          >
            {t}
          </button>
        ))}
      ...
      {tab === 'leaderboard' &&
        (leaderboard.length === 0 ? (
          <EmptyState
            ...
            description="Referral activity will appear here once users start referring."
          />
        ) : (
          <div className="bg-white rounded-2xl border border-warm-200 overflow-hidden">
            ...
            {leaderboard.map((r, i) => (
              <div
                key={r.referrerId}
                className="grid grid-cols-[40px_2fr_1fr_1fr_1fr] gap-4 px-6 py-4 border-t border-warm-100 items-center"
              >
                <span className={`text-sm font-bold ${i < 3 ? 'text-fox-500' : 'text-warm-400'}`}>
                  {i + 1}
                </span>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-fox-500/10 flex items-center justify-center text-fox-500 text-xs font-bold">
                    {getInitials(r.name)}
                  </div>
                ...
                <span className="text-sm font-mono font-semibold text-warm-900">
      ...
      {tab === 'payouts' &&
        (payouts.length === 0 ? (
          <EmptyState
            ...
            description="Commission payouts will appear here once referrals convert."
          />
        ) : (
          <div className="bg-white rounded-2xl border border-warm-200 overflow-hidden">
