        ...
        toast.success(
          (t) => (
            <span>
              Post created!{' '}
              <Link
                to={`/resources/${newPost.slug}`}
                target="_blank"
                className="text-fox-500 font-bold underline underline-offset-4"
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() => {
              setCurrentPost({ title: '', content: '', category: 'Engineering', status: 'draft' });
              setEditModal(true);
            }}
          >
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-warm-50">
                {filteredPosts.length === 0 && (
                  <tr>
                    <td colSpan="5" className="py-20 text-center text-warm-400 text-sm">
                      No posts found.
                    </td>
                  </tr>
                )}
                  ...
                  <tr key={post._id} className="hover:bg-warm-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm font-semibold text-warm-900 line-clamp-1">
                          {post.title}
                        </div>
                      </div>
                      <div className="text-[10px] text-warm-400 font-bold">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          ...
                          target="_blank"
                          className="p-1.5 hover:bg-warm-100 rounded-lg text-warm-400 hover:text-warm-700"
                        >
                          <Eye size={16} />
                        </Link>
                        <button
                          ...
                          className="p-2 hover:bg-fox-50 rounded-xl text-warm-400 hover:text-fox-600 transition-colors shadow-sm border border-transparent hover:border-fox-100 bg-white"
                        >
                          <Edit3 size={18} />
                        </button>
                        <button
                          onClick={() => handleDelete(post)}
                          className="p-2 hover:bg-red-50 rounded-xl text-warm-400 hover:text-red-600 transition-colors shadow-sm border border-transparent hover:border-red-100 bg-white"
                        >
                          <Trash2 size={18} />
                        </button>
                      </div>
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
            <div className="p-8 border-b border-warm-100 flex items-center justify-between">
              <h3 className="text-xl font-bold text-warm-900">
                {currentPost._id ? 'Edit Post' : 'Review & Save'}
              </h3>
              <button
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="col-span-2 flex items-center gap-3 bg-warm-50 p-4 rounded-2xl border border-warm-100">
                  ...
                  <div className="flex-1">
                    <label
                      htmlFor="featured"
                      className="text-sm font-bold text-warm-900 cursor-pointer block"
                    >
                      Featured Insight
                    </label>
                    <p className="text-[10px] text-warm-500 font-medium">
                      If checked, this post will appear in the "Latest Insights" section on the
                      homepage.
                    </p>
                  </div>
                  <Star
                    size={20}
                    className={
                      currentPost.featured ? 'text-amber-400 fill-amber-400' : 'text-warm-200'
                    }
                  />
                </div>
              </div>
