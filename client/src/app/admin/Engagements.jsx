...
export default function Engagements() {
  ...
  const setStatus = (e, status) => {
    ...
    api
      .patch(`/engagements/${e.id}/status`, { status })
    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Active engagements will appear here."
        />
      ) : (
        <div className="bg-white rounded-2xl border border-warm-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-warm-500 text-left bg-warm-50">
                ...
                {pageItems.map((e) => (
                  <tr key={e.id} className="border-t border-warm-50 hover:bg-warm-50/50">
                    ...
                    <td className="px-5 py-3">
                      <Link
                        to={`/app/admin/users?id=${e.clientId}`}
                        className="text-fox-600 hover:underline"
                      >
                        {e.clientId || '–'}
                      </Link>
                    </td>
                    ...
                    <td className="px-5 py-3 text-warm-500">
                      {formatDate(e.executedAt || e.createdAt)}
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex gap-2">
                        ...
                        <Link
                          to={`/app/admin/projects?clientId=${e.clientId}`}
                          className="text-xs px-2 py-1 rounded-lg bg-warm-100 text-warm-600 hover:bg-warm-200"
                        >
                          Projects
                        </Link>
                      </div>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <div className="text-xs text-warm-500">Model</div>
                <div className="font-medium">{selected.model}</div>
              </div>
              <div>
                <div className="text-xs text-warm-500">Status</div>
                <div className="font-medium">{selected.status}</div>
              </div>
              <div>
                <div className="text-xs text-warm-500">Client</div>
                <Link
                  to={`/app/admin/users?id=${selected.clientId}`}
                  className="text-fox-600 hover:underline"
                  onClick={() => setSelected(null)}
                >
                  {selected.clientId || '–'}
              ...
              <div>
                <div className="text-xs text-warm-500">Executed</div>
                <div className="font-medium">{formatDate(selected.executedAt)}</div>
              </div>
              <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {ENGAGEMENT_STATUSES.map((s) => (
                  ...
                  >
                    {actionLoading === `status-${selected.id}`
                      ? '...'
            ...
            <div>
              ...
              <Link
                to={`/app/admin/projects?clientId=${selected.clientId}`}
                onClick={() => setSelected(null)}
                className="text-fox-600 hover:underline text-sm"
              >
                View associated projects &rarr;
              </Link>
            </div>
          </div>
        </div>
