...
export default function RFPs() {
  ...
  const filtered = rfps.filter((r) => {
    ...
    if (
      search &&
      !r.issuer.toLowerCase().includes(search.toLowerCase()) &&
      ...
      <div className="flex gap-3">
        <div className="flex-1 relative">
          ...
          <input
            ...
            placeholder="Search by issuer or client..."
        ...
        <div className="bg-white rounded-2xl border border-warm-200 overflow-hidden">
          ...
          {filtered.map((r) => (
            <div key={r.id} className="border-t border-warm-100">
              <div
                className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_40px] gap-4 px-6 py-4 items-center cursor-pointer hover:bg-warm-50/50 transition"
              ...
              {expanded === r.id && (
                <div className="px-6 pb-4">
                  <div className="bg-warm-50 rounded-xl p-4 text-sm text-warm-700">
                    {r.lossReason ? (
                      <p className="mb-2">
                        <span className="font-semibold">Loss reason:</span> {r.lossReason}
                      </p>
                    ) : null}
                    {r.deadline ? (
                      <p className="mb-2">
                        <span className="font-semibold">Deadline:</span> {formatDate(r.deadline)}
                      </p>
                    ) : null}
