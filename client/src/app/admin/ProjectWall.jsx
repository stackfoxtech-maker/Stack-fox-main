...
export default function AdminProjectWall() {
  ...
  const filtered = inquiries.filter((q) => {
    ...
    if (
      search &&
      !`${q.name} ${q.company || ''} ${q.email}`.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((q) => (
            <div
              key={q.id}
              className="bg-white rounded-2xl border border-warm-200 p-5 hover:shadow-md transition-shadow"
            >
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-warm-500 mt-0.5 uppercase tracking-wider truncate">
                    {q.company || 'No company'}
                  </p>
                </div>
                <Badge variant={statusColors[q.status] || 'neutral'}>
                  {capitalize((q.status || '').toLowerCase())}
                </Badge>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                {updatingId === q.id && (
                  <Loader2 size={14} className="animate-spin text-warm-400" />
                )}
              </div>
            </div>
          ))}
