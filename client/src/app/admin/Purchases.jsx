...
export default function Purchases() {
  ...
  const fetchItems = () => {
    setLoading(true);
    api
      .get('/admin/purchases', { params: { limit: 100, q: query || undefined } })
  ...
  const renderClientCard = (client, contact) => (
    <div className="bg-warm-50 rounded-xl p-4 border border-warm-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
        <div>
          <div className="text-[10px] text-warm-400 uppercase tracking-wider mb-1">
            Organization
          </div>
        ...
        <div>
          ...
          <div className="font-medium text-warm-800">
            {client?.type || '–'} {client?.tier ? `• ${client?.tier}` : ''}
          </div>
        </div>
        <div>
          <div className="text-[10px] text-warm-400 uppercase tracking-wider mb-1">GSTIN</div>
          <div className="font-mono text-xs text-warm-700">
            {client?.gstin || 'Not provided'}{' '}
            {client?.gstinVerified && (
              <span className="text-emerald-600 text-[10px] ml-1">✓ Verified</span>
            )}
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <Badge
            variant={
              client?.healthState === 'ACTIVE'
                ? 'success'
                : client?.healthState === 'DORMANT'
                  ? 'warning'
                  : 'neutral'
            }
            className="text-[10px]"
          >
            {client?.healthState || 'N/A'}
          </Badge>
        </div>
        <div>
        ...
        {contact && (
          <>
            ...
            <div>
              ...
              <div className="text-[10px] text-warm-400 uppercase tracking-wider mb-1 mt-2">
                Contact Phone
              </div>
              <div className="text-xs text-warm-700">
      ...
      {items && items.length > 0 ? (
        <div className="space-y-2">
          {items.map((sub, i) => (
            <div
              key={i}
              className="flex justify-between items-center bg-white p-3 rounded-lg border border-warm-100/60"
            >
              <div>
                <div className="text-sm font-bold text-warm-900">
                  {sub.name || sub.serviceId || 'Item'}
                </div>
                <div className="text-[10px] text-warm-500">
                  Qty: {sub.quantity || 1} {sub.serviceId ? `• ID: ${sub.serviceId}` : ''}
                </div>
              </div>
            </div>
          ))}
            ...
            return (
              <div key={c.id} className="bg-white p-3 rounded-lg border border-warm-100/60">
                ...
                <div className="flex items-center gap-3 text-[10px] text-warm-500">
                  ...
                  <span
                    className={`flex items-center gap-1 ${clientSig ? 'text-emerald-600' : 'text-warm-400'}`}
                  >
                    {clientSig ? <CheckCircle2 size={10} /> : <Clock size={10} />} Client{' '}
                    {clientSig ? 'signed' : 'pending'}
                  </span>
                  <span
                    className={`flex items-center gap-1 ${sfSig ? 'text-emerald-600' : 'text-warm-400'}`}
                  >
          ...
          {projects.map((p) => (
            <div key={p.id} className="bg-white p-3 rounded-lg border border-warm-100/60">
              <div className="flex items-center justify-between mb-1">
                ...
                <Badge variant={getStatusBadgeVariant(p.status)} className="text-[10px]">
                  {p.status}
                </Badge>
              </div>
              <div className="text-xs text-warm-500">
                Service: {p.service?.name || p.serviceId || '–'}
              </div>
        ...
        <div className="space-y-2">
          {payments.map((pay) => (
            ...
            >
              ...
              <Badge variant={getStatusBadgeVariant(pay.status)} className="text-[10px]">
                {pay.status}
              </Badge>
            </div>
          ))}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-warm-500">
            Unified view of every client purchase with contracts, items, and client details.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Completed purchases will appear here."
        />
      ) : (
        <div className="space-y-3">
          {items.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-[2rem] border border-warm-200 overflow-hidden transition-all duration-300"
            >
              <div
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h3 className="font-bold text-warm-900">
                          {item.kind === 'quote' ? item.quoteNumber : item.id}
                        </h3>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-warm-100 text-warm-600 uppercase">
                          {item.kind}
                        </span>
                      ...
                      <div className="flex items-center gap-3 text-xs text-warm-500 flex-wrap">
                        ...
                        {item.client?.contactEmail && (
                          <>
                            ...
                            <span>{item.client.contactEmail}</span>
                          </>
                        )}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right mr-2">
                      <div className="text-[10px] text-warm-400 font-bold uppercase tracking-widest">
                        Total
                      </div>
              ...
              {expandedId === item.id && (
                <div className="px-5 md:px-6 pb-6 pt-2 border-t border-warm-100 animate-slide-up">
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    {item.client && (
                      <Link
                        to={`/app/admin/users?id=${item.client.id}`}
                        className="btn-outline py-2 px-4 rounded-xl text-xs gap-2 inline-flex items-center"
                      >
                    ...
                    {(item.contact?.email || item.client?.contactEmail) && (
                      <a
                        href={`mailto:${item.contact?.email || item.client?.contactEmail}`}
                        className="btn-outline py-2 px-4 rounded-xl text-xs gap-2 inline-flex items-center"
                      >
