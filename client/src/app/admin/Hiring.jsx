...
export default function Hiring() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      selectedJob === 'all'
        ...
        : api.get(`/jobs/${selectedJob}/applications`),
    ])
      .then(([jr, ar]) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {jobs.map((j) => (
          <button
            ...
            className={cn(
              'bg-white rounded-xl border border-warm-200 p-4 text-left hover:shadow-card transition-shadow',
              selectedJob === j._id && 'ring-2 ring-fox-500',
            )}
          >
            <p className="text-sm font-medium text-warm-900 truncate">{j.title}</p>
            <p className="text-xs text-warm-500 mt-1">
              {j.type} &middot; {j.applicationCount || 0} applications
            </p>
          </button>
        ))}
      </div>
      ...
      {selectedJob === 'all' ? (
        <div className="bg-warm-100 rounded-xl p-8 text-center text-warm-500">
        ...
        <div className="space-y-3">
          {apps.map((a) => (
            <div key={a._id} className="bg-white rounded-xl border border-warm-200 p-4">
              <div className="flex items-start justify-between gap-3 mb-2">
                <div>
                  ...
                  <p className="text-xs text-warm-500">
                    {a.email} &middot; {a.experience || 'N/A'} &middot; Applied{' '}
              ...
              {a.coverLetter && (
                <p className="text-xs text-warm-600 mb-2 line-clamp-2">{a.coverLetter}</p>
              )}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      a.status === s
                        ...
                        : 'bg-warm-100 text-warm-600 hover:bg-fox-50 hover:text-fox-600',
                    )}
                  >
                    {capitalize(s)}
                  </button>
