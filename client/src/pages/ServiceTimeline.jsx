...
const TIERS = [
  ...
  },
  {
    id: 'PREMIUM',
    name: 'Premium',
    tag: 'Dedicated team, white-glove',
  ...
  const stages = [
    {
      ...
      span: '1-2 days',
      desc: 'We understand your goal, review requirements, and send a kickoff checklist.',
    },
    {
      n: 2,
      name: 'Design',
      span: '20% of timeline',
      desc: 'Wireframes and UI designs. One revision round included (Starter) or two (Growth).',
    },
    {
      n: 3,
      name: 'Development',
      span: Math.round(estDays * 0.5) + '-' + Math.round(estDays * 0.6) + ' days',
      desc: 'Core building phase. You get a staging link to follow progress.',
    },
    {
      n: 4,
      name: 'Testing and QA',
      span: '2-3 days',
      desc: 'Cross-browser, responsive, performance and SEO checks.',
    },
    {
      n: 5,
      name: 'Review and Revisions',
      span: '1-3 days',
      desc: 'You review the work and request changes within the included rounds.',
    },
    {
      n: 6,
      name: 'Deployment and Handover',
      span: '1-2 days',
      desc: 'Live deployment, documentation, credentials, and source code handover.',
    },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      <nav className="text-sm text-gray-500 mb-6 flex flex-wrap gap-1 items-center">
        ...
        <span>{'>'}</span>
        <Link to={`/services/${category}`} className="hover:text-orange-600">
          {cat?.name}
        </Link>{' '}
        <span>{'>'}</span>
      ...
      <p className="text-lg text-gray-600 mb-8">
        Typical delivery: <strong>{service.est}</strong>. Rush delivery is available at a 20-40%
        acceleration.
      </p>
            ...
            <div className="bg-white border rounded-xl p-5">
              <div className="flex items-center justify-between flex-wrap gap-2 mb-1">
                <h3 className="font-bold text-lg">
                  <span className="text-orange-600">Stage {s.n}</span> — {s.name}
                </h3>
              ...
              {i === 0 && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  <Link
                    to={`/services/${category}/${service.slug || service.id}`}
                    className="text-sm text-orange-600 font-semibold hover:underline"
                  >
                    View full service
                  </Link>
                  <Link
                    to={`/services/${category}/${slug}/cost`}
      ...
      <div className="bg-[#FAFAF8] rounded-2xl p-8 text-center">
        ...
        <p className="text-gray-600 mb-6">
          We offer rush delivery (up to 50% faster). Ask in the Builder or use the estimator.
        </p>
