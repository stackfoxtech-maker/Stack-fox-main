  ...
  if (!bundle) {
    return (
      <div className="max-w-3xl mx-auto px-6 py-24 text-center">
        ...
        <Link to="/packages" className="text-orange-600 font-semibold">
          Browse all bundles
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-16">
      <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2">
        ...
        <span>{'>'}</span>
        <Link to="/packages" className="hover:text-orange-600">
          Bundles
        </Link>
        <span>{'>'}</span>
      ...
      <p className="text-gray-600 mb-6 max-w-2xl">
        {bundle.desc || 'Complete solution bundle curated for your industry.'}
      </p>
      ...
      <div className="bg-[#FAFAF8] rounded-2xl p-8 text-center mb-12">
        ...
        <div className="text-sm text-gray-500 mt-1">
          {bundle.est || 'Delivered in 15-30 days'} · {bundle.serviceIds?.length || 6} services
          included
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          to={'/checkout/express?service=' + bundle.id}
