...
const QUESTIONS = [
  {
    id: 'q1',
    q: 'What do you want to build or improve?',
    options: [
      ...
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="bg-[#FAFAF8] border rounded-2xl p-6 mb-8">
          <div className="flex items-center gap-6">
            ...
            <div
              className={`text-2xl font-extrabold ${result.tier === 'PREMIUM' ? 'text-purple-600' : result.tier === 'GROWTH' ? 'text-blue-600' : 'text-orange-600'}`}
            >
              {result.tier}
            </div>
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-16">
      <p className="text-sm font-semibold text-orange-600 mb-2">
        Pre-purchase Quiz · {step + 1} of {QUESTIONS.length}
      </p>
      ...
      <div className="space-y-3">
        {q.options.map((opt, i) => (
          <button
            key={i}
            onClick={() => choose(opt)}
            className="w-full text-left bg-white border-2 border-gray-200 rounded-xl px-5 py-4 font-medium hover:border-orange-500 hover:text-orange-600 transition-colors"
          >
            {opt.label}
          </button>
        ))}
