...
const steps = [
  {
    ...
    desc: 'Copy your unique referral link and share it with businesses you know.',
  },
  {
    icon: Users,
    title: 'They Sign Up',
    desc: 'When someone signs up and makes their first purchase using your link, it counts.',
  },
  {
    icon: IndianRupee,
    title: 'You Earn ₹5,000',
    desc: 'Get ₹5,000 credited for every successful referral. No cap on earnings.',
  },
];
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-warm-500 max-w-lg mx-auto">
            Earn ₹5,000 for every business you refer to StackFox. No limits, no strings.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-6 mb-6">
          ...
          <div className="flex gap-2">
            <input
              readOnly
              value={link}
              className="flex-1 bg-warm-50 border border-warm-200 rounded-lg px-4 py-2.5 text-sm text-warm-700 font-mono"
            />
            <button
              onClick={copyLink}
              className="flex items-center gap-1.5 bg-fox-500 text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-fox-600 transition"
            >
              {copied ? (
                <>
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-6">
          {[
            ...
            {
              label: 'Converted',
              value: mockReferrals.filter((r) => r.status === 'Converted').length,
            },
            ...
            {mockReferrals.map((r, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusStyle[r.status]}`}
                  >
                    {r.status}
                  </span>
                  {r.earned > 0 && (
        ...
        <div className="mt-6 bg-fox-500/5 border border-fox-500/20 rounded-2xl p-5 text-sm text-warm-700">
          ...
          <p>
            Earnings are paid out monthly via UPI or bank transfer. Minimum payout threshold is
            ₹5,000. Payouts are processed on the 1st of every month for the previous month's
            conversions.
          </p>
        </div>
      </div>
