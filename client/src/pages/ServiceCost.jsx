    ...
    <div className="max-w-5xl mx-auto px-6 py-16">
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-12">
        {TIERS.map((t) => {
          ...
          return (
            <div
              key={t.id}
              className={`border-2 ${t.card} bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col`}
            >
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-bold">{t.name}</h3>
                <span className={`text-xs font-medium px-2 py-1 rounded-full ${t.text} bg-gray-50`}>
                  {t.tag}
                </span>
              </div>
      ...
      <div className="bg-gradient-to-br from-fox-500 to-fox-600 rounded-2xl p-8 text-center mb-12 shadow-lg shadow-fox-500/20">
        ...
        <p className="text-white/85 mb-6 max-w-md mx-auto">
          Answer 10 quick questions and let our AI advisor recommend the perfect services for your
          project.
        </p>
        <div className="flex flex-wrap justify-center gap-4">
          <Link
            to="/advisor"
            className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 font-semibold text-fox-600 shadow transition hover:bg-white/90 hover:shadow-lg"
          >
            <Sparkles size={18} /> Start Advisor
          </Link>
          <Link
            to="/builder"
            className="inline-flex items-center gap-2 rounded-full border-2 border-white/40 px-6 py-3 font-semibold text-white transition hover:bg-white/10"
          >
            Open the Builder
          </Link>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {services
          .filter((s) => s.catId === category && s.id !== service.id)
          .slice(0, 6)
          .map((s) => (
            <Link
              key={s.id}
              to={`/services/${category}/${s.slug || s.id}`}
