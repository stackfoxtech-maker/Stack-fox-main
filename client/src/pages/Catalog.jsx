...
export default function Catalog() {
  ...
  const filtered = useMemo(() => {
    ...
    if (priceRange === 'under10k') result = result.filter((s) => s.price < 10000);
      ...
      result = result.filter((s) => s.price >= 10000 && s.price <= 25000);
    else if (priceRange === '25k-50k')
      result = result.filter((s) => s.price >= 25000 && s.price <= 50000);
    else if (priceRange === 'above50k') result = result.filter((s) => s.price > 50000);
    if (q) {
      const lower = q.toLowerCase();
      result = result.filter(
        (s) => s.name.toLowerCase().includes(lower) || s.lay?.toLowerCase().includes(lower),
      );
    }
    return result;
  ...
  if (error)
    return (
      <Section>
        <p className="text-danger-600">Failed to load catalogue.</p>
      </Section>
    );
              ...
              <div className="space-y-1">
                <button
                  ...
                  className={cn(
                    ...
                    activeCat === 'all'
                      ? 'bg-fox-50 text-fox-600 font-medium'
