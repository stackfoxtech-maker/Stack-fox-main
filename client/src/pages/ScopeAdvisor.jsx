...
const QUESTIONS = [
  {
    key: 'industry',
    label: 'What industry are you in, and what is your business model?',
    placeholder: 'e.g. D2C e-commerce, B2B SaaS, healthcare clinic, marketplace, consultancy',
  },
  {
    key: 'problem',
    label: 'What is the core problem you want to solve, and what does success look like?',
    placeholder: 'e.g. No online presence, manual order tracking, 40% cart abandonment',
  },
  {
    key: 'currentState',
    label: 'Are you building something new, or improving an existing product/site?',
    placeholder: 'New build / Existing — describe current pain points and tech if any',
  },
  {
    key: 'users',
    label: 'Who are your primary users, and what scale are you planning for?',
    placeholder: 'e.g. 500 visitors/month, 50 orders/day, 10k MAU in year 1',
  },
  {
    key: 'platforms',
    label: 'Which platforms do you need?',
    placeholder: 'e.g. Responsive web, iOS, Android, WhatsApp bot, desktop app',
  },
  {
    key: 'features',
    label: 'What are the must-have features for launch vs nice-to-haves for later?',
    placeholder: 'Must-have: login, payments, dashboard. Nice-to-have: AI chat, referrals',
  },
  {
    key: 'integrations',
    label: 'Which external tools/services must this connect to?',
    placeholder: 'e.g. Razorpay/UPI, WhatsApp Business API, Google Sheets, Salesforce, Tally',
  },
  {
    key: 'compliance',
    label: 'Do you have any compliance, data, or security requirements?',
    placeholder: 'e.g. GST invoicing, GDPR, SOC2, data residency in India, role-based access',
  },
  {
    key: 'constraints',
    label: 'What is your budget range and desired go-live timeline?',
    placeholder: 'e.g. Under ₹3L, launch in 6–8 weeks, phased rollout OK',
  },
  {
    key: 'support',
    label: 'Will you need ongoing support after launch, and who will own this internally?',
    placeholder: 'e.g. Monthly retainer, our marketing team will manage, no internal dev',
  },
];
...
export default function ScopeAdvisor() {
  ...
  const catalogSlim = SF_DATA.services.map((s) => ({
    id: s.id,
    name: s.name,
    catId: s.catId,
    price: s.price,
      ...
      <div className="max-w-2xl mx-auto py-12 px-4">
        ...
        <div className="bg-white rounded-3xl border-2 border-fox-300 p-6 mb-6">
          ...
          <div className="space-y-2 mb-4">
            {advice.itemIds.map((id) => {
              ...
              return s ? (
                <div
                  key={id}
                  className="flex justify-between text-sm border-b border-warm-100 pb-2"
                >
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="primary"
              onClick={() => loadIntoBuilder(advice.itemIds)}
              className="gap-2"
            >
              {isStarterMatch
                ? `Get Started — ${formatINR(totalFor(advice.itemIds))}`
                : 'Load into Builder'}{' '}
              <ArrowRight size={16} />
            </Button>
          </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {advice.lighterAlt?.itemIds?.length > 0 && (
            <div className="bg-warm-50 rounded-2xl p-4 border border-warm-200">
              <h3 className="text-xs font-bold text-warm-500 uppercase tracking-wide mb-2">
                Lighter alternative
              </h3>
              <p className="text-sm text-warm-600 mb-3">{advice.lighterAlt.rationale}</p>
              <button
                onClick={() => loadIntoBuilder(advice.lighterAlt.itemIds)}
                className="text-xs font-bold text-fox-600 hover:underline"
              >
          ...
          {advice.heavierAlt?.itemIds?.length > 0 && (
            <div className="bg-warm-50 rounded-2xl p-4 border border-warm-200">
              ...
              <button
                onClick={() => loadIntoBuilder(advice.heavierAlt.itemIds)}
                className="text-xs font-bold text-fox-600 hover:underline"
              >
  ...
  return (
    <div className="max-w-lg mx-auto py-16 px-4">
      ...
      <div className="h-1.5 bg-warm-100 rounded-full mb-8">
        <div
          className="h-full bg-fox-500 rounded-full transition-all"
