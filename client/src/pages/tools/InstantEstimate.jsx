            ...
            <div className="space-y-3">
              {items.map((s) => (
                <label key={s.id} className="flex items-center gap-3 cursor-pointer group">
                  <div
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center transition ${selected.has(s.id) ? 'bg-fox-500 border-fox-500' : 'border-warm-300'}`}
                  >
      ...
      {base > 0 && (
        <div className="grid md:grid-cols-3 gap-6">
          {tiers.map((t) => (
            <div
              key={t.name}
              className={`bg-white rounded-2xl border p-6 text-center ${t.name === 'Growth' ? 'border-fox-500 ring-2 ring-fox-500/20' : 'border-warm-200'}`}
            >
              {t.name === 'Growth' && (
              ...
              <div className="text-3xl font-bold text-warm-900">
                ${Math.round(base * t.mult).toLocaleString()}
              </div>
              <button className="mt-4 bg-fox-500 text-white rounded-xl px-6 py-3 hover:bg-fox-600 transition w-full">
                Get Started
              </button>
            </div>
          ))}
