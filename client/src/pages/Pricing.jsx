    ...
    <>
      <Section>
              ...
              <div key={cat.id}>
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <ArrowRight
                      size={15}
                      className="mt-1 text-warm-300 transition-colors group-hover:text-fox-500"
                    />
                  </div>
        ...
        <SectionHeading
          ...
          description="Curated bundles that save 15–30% versus picking the pieces one by one."
          center={false}
        />
          ...
          <table className="w-full min-w-[36rem] text-body-sm">
            ...
            <tbody>
              {data.packages.map((pkg) => (
                <tr
                  key={pkg.id}
                  className="border-b border-warm-100 transition-colors hover:bg-warm-50"
                >
                  <td className="py-3.5 pr-4">
                    <span className="font-medium text-warm-900">{pkg.name}</span>
                    {pkg.popular && (
                      <span className="badge-fx badge-fox ml-2 text-caption">Popular</span>
                    )}
                  </td>
                  <td className="py-3.5 px-4 text-right price-tag text-warm-900">
                    {formatINR(pkg.price)}
                  </td>
                  <td className="py-3.5 px-4 text-right price-tag text-sage-700">
                    {formatINR(pkg.savings)}
                  </td>
        ...
        <div className="mx-auto max-w-2xl space-y-2.5">
          {data.faq.map((item, i) => {
            ...
            return (
              <div
                key={i}
                className="overflow-hidden rounded-md border border-warm-200 bg-warm-white"
              >
                <button
                ...
                >
                  {item.q}
                  {open ? (
                    <Minus size={16} className="shrink-0 text-fox-500" />
                  ) : (
                    <Plus size={16} className="shrink-0 text-warm-400" />
                  )}
                </button>
                {open && (
                  <div className="animate-fade-in px-5 pb-4 text-body-sm leading-relaxed text-warm-600">
                    {item.a}
                  </div>
                )}
              </div>
