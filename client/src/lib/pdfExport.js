...
export const exportQuotePDF = async (
  ...
  // ── Meta row ──
  y = 36;
  const metaL = (t, x, yy, o) => {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(6);
    sc(...C.light);
    doc.text(t, x, yy, o);
  };
  const metaV = (t, x, yy, o) => {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    sc(...C.dark);
    doc.text(t, x, yy, o);
  ...
  metaL('DATE', ML, y);
  metaV(quoteDate, ML, y + 4.5);
  metaL('VALID UNTIL', ML + 45, y);
  metaV(validUntil || 'On request', ML + 45, y + 4.5);
  metaL('TIER', ML + 95, y);
  metaV(tier, ML + 95, y + 4.5);
  metaL('STATUS', W - MR, y, { align: 'right' });
  metaV(status, W - MR, y + 4.5, { align: 'right' });
  ...
  const cn =
    quote.checkoutDetails?.account?.name || quote.checkoutDetails?.account?.companyName || '';
  ...
  if (cn) {
    ...
    if (ce) {
      ...
      doc.setFontSize(7);
      sc(...C.mid);
      doc.text(ce, W - MR, y + 9, { align: 'right' });
    }
  }
  ...
  items.forEach((x, i) => {
    ...
    const typeLabel =
      info?.type === 'package' ? 'Package' : info?.type === 'addon' ? 'Add-on' : 'Service';
  ...
  sc(...C.dark);
  doc.text(fmtCur(sub), sX + sW, y, { align: 'right' });
  y += 5.5;
  sc(...C.mid);
  doc.text(cur.taxName + ' @ ' + cur.tax + '%', sX, y);
  sc(...C.dark);
  doc.text(fmtCur(tx), sX + sW, y, { align: 'right' });
  y += 4;
  sd(...C.fox);
  ...
  sc(...C.mid);
  doc.text(
    'Complete breakdown of every service and package included in this quotation.',
    ML,
    y + 6,
  );
  y += 14;
  ...
  items.forEach((x, idx) => {
    ...
    if (isPackage && pkg?.items?.length > 0) {
      ...
      pkg.items.forEach((subId, si) => {
        ...
        sc(...C.light);
        doc.text(
          'Est: ' + (subSvc.est || '-') + '  |  ' + fmtCur(subSvc.price),
          W - MR - 4,
          y + 1.5,
          { align: 'right' },
        );
      ...
      if (pkg.savings) {
        ...
        sc(...C.green);
        doc.text(
          'You save ' + fmtCur(pkg.savings) + ' with this package vs buying individually',
          ML + 8,
          y + 2.5,
        );
        y += 9;
  ...
  const terms = [
    ...
    '2.  All amounts are in ' +
      cur.code +
      '. GST @18% is applied as per Indian tax regulations where applicable.',
  ...
  sc(...C.text);
  doc.text(
    'Reply to this quote or contact us at stackfox.tech@gmail.com  |  +91 82093 95894',
    ML + 35,
    y + 6,
  );
...
export async function exportTaxInvoicePDF(inv) {
  ...
  const T = (s, x, y, o = {}) => {
    ...
    doc.text(String(s), x, y, {
      align: o.align || 'left',
      charSpace: o.spread || 0,
      maxWidth: o.maxWidth,
    });
  };
  const rule = (y, w = 0.5, c = RULE) => {
    sd(c);
    doc.setLineWidth(w);
    doc.line(M, y, W - M, y);
  };
  ...
  T('ORIGINAL FOR RECIPIENT', W - M - 22, y - 1, {
    align: 'center',
    size: 6,
    color: BLUE,
    bold: true,
  ...
  label('SUPPLIER', c1, y);
  let sy = y + 5;
  T(inv.supplier.legalName, c1, sy, { bold: true, size: 8 });
  sy += 4;
  T(inv.supplier.tradeName, c1, sy, { size: 7, color: BLUE });
  sy += 4;
  inv.supplier.addressLines.forEach((l) => {
    T(l, c1, sy, { size: 7.5, color: MUT });
    sy += 3.8;
  });
  T(`Ph: ${inv.supplier.phone}`, c1, sy, { size: 7.5, color: MUT });
  sy += 3.8;
  T(inv.supplier.email, c1, sy, { size: 7.5, color: MUT });
  sy += 3.8;
  T(inv.supplier.website, c1, sy, { size: 7.5, color: MUT });

  label('RECIPIENT', c2, top);
  let ry = top + 5;
  T(inv.recipient.name, c2, ry, { bold: true, size: 8 });
  ry += 4;
  if (inv.recipient.contact) {
    T(`Attn: ${inv.recipient.contact}`, c2, ry, { size: 7.5, color: MUT });
    ry += 3.8;
  }
  if (inv.recipient.address) {
    T(inv.recipient.address, c2, ry, { size: 7.5, color: MUT, maxWidth: CW * 0.3 });
    ry += 3.8;
  }
  if (inv.recipient.email) {
    T(inv.recipient.email, c2, ry, { size: 7.5, color: MUT });
    ry += 3.8;
  }
  if (inv.recipient.phone) {
    T(inv.recipient.phone, c2, ry, { size: 7.5, color: MUT });
    ry += 3.8;
  }
  if (inv.recipient.gstin) {
    T(`GSTIN: ${inv.recipient.gstin}`, c2, ry, { size: 7.5, color: MUT });
    ry += 3.8;
  }
  if (inv.recipient.stateName)
    T(`State: ${inv.recipient.stateName} (${inv.recipient.stateCode})`, c2, ry, {
      size: 7.5,
      color: MUT,
    });
  ...
  y += 5;
  T(`Beneficiary: ${inv.bank.beneficiary}`, M, y, { size: 7.5 });
  y += 4;
  T(`Bank: ${inv.bank.bank}`, M, y, { size: 7.5 });
  y += 4;
  T(`Branch: ${inv.bank.branch}`, M, y, { size: 7.5 });
  y += 4;
  T(`PAN: ${inv.bank.pan}`, M, y, { size: 7.5 });

  let gy = sigTop;
  T(`FOR ${inv.supplier.legalName}`, W - M, gy, {
    bold: true,
    size: 7.5,
    color: BLUE,
    align: 'right',
  ...
  T(`Date: ${inv.date}   Place: ${inv.place}`, W - M, gy + 8.5, {
    size: 6.5,
    color: MUT,
    align: 'right',
  });
  T('Digitally signed — IT Act 2000, Sec. 5', W - M, gy + 12.5, {
    size: 6,
    color: MUT,
    align: 'right',
  });
  ...
  rule(fy - 4, 0.8, BLUE);
  T(
    `${inv.supplier.legalName}  |  ${inv.supplier.tradeName}  |  CIN: ${inv.supplier.cin}  |  GSTIN: ${inv.supplier.gstin}  |  PAN: ${inv.supplier.pan}`,
    W / 2,
    fy,
    { align: 'center', bold: true, size: 6 },
  );
  T(
    `${inv.supplier.addressLines.join(', ')}  ·  ${inv.supplier.phone}  ·  ${inv.supplier.email}  ·  ${inv.supplier.website}`,
    W / 2,
    fy + 3.5,
    { align: 'center', size: 5.5, color: MUT },
  );
